...
- **100 % browser-side PBF parsing**: the backend serves the raw bytes at `GET /examples/data/lower_mnt.osm.pbf`; DuckDB-WASM in the browser handles all OSM parsing — no server-side processing.
- **Relative data-source URLs**: the spec writes `"pbfFileUrl": "examples/data/lower_mnt.osm.pbf"` (no host/port) and the lifecycle prepends `BACKEND_URL` automatically at run time.
