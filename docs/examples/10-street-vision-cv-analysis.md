...
!!! note "Setup required"
    Before running, install the streetvision Python extras (`pip install "utk-curio[streetvision]"`), set the `GOOGLE_MAPS_API_KEY` env var in your backend environment, and install the **Street Vision** package from Curio's `/catalog` page. The Spatial Join node is built-in and needs no separate install.
