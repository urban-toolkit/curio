...
- **Presence.** Open the side panel from the 👥 button in the top bar (next to the **AI assistance** button). It lists everyone currently in the project room, with a connection indicator.
...
| [components/menus/top/UpMenu.tsx](../utk_curio/frontend/urban-workflows/src/components/menus/top/UpMenu.tsx) | 👥 trigger button (next to **AI assistance**) that toggles the side panel; only rendered when `collab.enabled`. |
