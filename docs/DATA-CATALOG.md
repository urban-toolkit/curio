...
- [5. Importing, publishing, and sharing](#5-importing-publishing-and-sharing) — supported formats, OSM PBF, publish / unpublish / delete.
...
A **dataset** in Curio is a small self-contained folder — the same shape as a node package — identified by a reverse-domain id and a major version:
...
Ids are 2–6 dot-separated lowercase segments (`[a-z][a-z0-9-]*`, ≤63 chars each), with a major version of `0`–`9999`. Curio mints ids for you in the non-hub cases:
...
|---|---|---|
| `id` | ✅ | Dataset id (see grammar above). |
| `name` | ✅ | Display title. |
| `version` | ✅ | Free-form version string (e.g. `"1.0.0"`), independent of `compatibility.major`. |
| `format` | ✅ | One of `csv`, `geojson`, `json`, `parquet`, `geotiff`, `shp`, `bundle`. |
| `dataFile` | ✅ | Path to the data relative to the dataset folder, e.g. `data/chicago.geojson`. |
| `compatibility.major` | — | Integer major version; defaults to `1`. Together with `id` it forms the directory name. |
| `description` | — | Defaults to `""`. |
| `publisher` | — | Defaults to `"Data Catalog"`. **Load-bearing**: it decides who may unpublish or delete (see *Operator notes*). |
...
| `featureCount` / `rowCount` | — | Counts for geo and tabular data respectively. |
| `schema` | — | Object describing fields; inferred from a preview when absent. |
| `groupId` / `layerName` | — | Multi-part imports (OSM PBF): every layer of one import shares a `groupId`. |
| `producerNodeId`, `producerNodeType`, `producerDataflowId`, `producerDataflowName`, `upstreamInputs` | — | Lineage for computed datasets (see [part 4](#4-computed-datasets-node-outputs)). |
...
> [!NOTE]
> **"Bundle" here means a multi-part dataset — nothing to do with the webpack/JS bundles discussed in [EXTENDING.md](EXTENDING.md) and [DEPLOYMENT.md](DEPLOYMENT.md).**
...
The dataset detail view — the `/catalog/data/<id>` page, or the modal opened from a drawer card — has four tabs: **Overview**, **Schema**, **Table Preview**, and **Lineage**. Previews page six rows at a time and are capped server-side at 500 rows per request.
...
The shared catalog defaults to `<repo_root>/datasets/`, resolved relative to the installed package. That is correct when Curio runs from a checkout, but on a `pip` install it resolves under `site-packages` (read-only — publishing fails), and in Docker it is not persisted across restarts. Set **`CURIO_CATALOG_ROOT`** (or pass `--catalog-root`) to a writable, persistent path in those deployments.
...
- [`utk_curio/backend/app/datasets/`](../utk_curio/backend/app/datasets/) — the implementation; [`domain/manifest.py`](../utk_curio/backend/app/datasets/domain/manifest.py) is the manifest contract.
