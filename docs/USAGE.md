...
| `--exec-memory-mb` / `--exec-timeout` / `--exec-parallelism` | 4096 / 300 / host-sized | Limits for isolated execution. `exec-memory-mb` is what a node may allocate on top of the interpreter its child starts with, with a floor of 64. The real memory ceiling is `exec-memory-mb x exec-parallelism`, which is why the parallelism default is derived rather than fixed: the smallest of half the cores, half the memory this process can see divided by `exec-memory-mb`, and what the container's PID limit can carry, floor 2, ceiling 32. In a container it reads the cgroup limit, not the host's RAM |
