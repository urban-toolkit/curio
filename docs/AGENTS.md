...
> This document covers the agent **runtime**: the manifest contract, capabilities, the provider, where lifecycle state lives, and the `/api/agents` endpoints. For the catalog surfaces built on top of it - the drawer, the browse page, the palette, and which layer each action writes - see [AGENT-CATALOG.md](AGENT-CATALOG.md).
...
Open the drawer from the top menu: **Data → Agent Catalog**. It has three scope tabs - **Browse all**, **My imports**, **In dataflow** - reusing the same drawer chrome as the Node and Data catalogs. The agents added to the open dataflow also appear in the **Agent Catalog** palette in the left tools panel (alongside Node Catalog and Data Catalog), with a **Browse Agent Catalog +** shortcut back to the drawer; palette rows are draggable and action-free. The catalog model itself - the storage layers, the action matrix, and the browse page at `/catalog/agents` - is documented in [AGENT-CATALOG.md](AGENT-CATALOG.md).
...
| `requiresAgents` | | A **subset of `delegatesTo`**: agents a *server* code path of this agent invokes without model choice (the Dataflow Builder's Solve → `node.content.generate`). The user's explicit Install adds the closure in one write, or refuses naming what cannot be resolved; uninstalling one while a dependent is installed is refused (memo dev/106, DEC-068). |
...
`requiresAgents` (dev/106) narrows that: the few delegates an agent is **not functional without**, because a server code path - not the model - invokes their capability. It is validated as a subset of `delegatesTo`. The distinction matters at install time: the orchestrator still never installs anything silently (`REQ-ORCH-001`), but the *user's* Install click resolves the required closure (`delegation.required_closure`) before writing, installs root and closure in one lockfile write, and refuses with the unresolvable ids otherwise; the catalog card discloses the dependency and the button says what it adds. The other delegates stay optional and are never fanned out.

---

## 4. The default LLM provider

An agent's `providerRequirements` declare what it needs from a provider (e.g. `structured-output`); the *actual* provider, model, endpoint, and key come from configuration, not the manifest. Curio resolves this in one place, [`app/agents/provider_config.py`](../utk_curio/backend/app/agents/provider_config.py)::`resolve_provider_config`, so there is no second built-in default scattered through the code.

**Curio ships with no default endpoint.** An earlier build defaulted to a specific university endpoint, which meant an unconfigured instance silently sent user data to a third party. Today the shipped defaults are empty and an unconfigured install reaches a clear "no provider configured" error, with every blocked agent surface linking to **AI Settings**.

| Setting | Env var | Launcher flag | Shipped default |
|---|---|---|---|
...
- A user who has configured their own provider under **AI Settings** keeps that exact config; the default never overrides or contaminates it (a configured Anthropic user never picks up the default base URL).
...
Under `CURIO_TESTING`, the port also accepts `api_type == "testing"`, a scripted provider ([`app/agents/testing_provider.py`](../utk_curio/backend/app/agents/testing_provider.py)) that replays a fixed reply table and reports fixed token counts. It is what the e2e suite and the screenshot gallery drive, so the ledger and quota paths stay exercised without a network call. The guard is re-checked at call time, not just at import.
...
| **In dataflow** (per-dataflow lockfile) | `spec["dataflow"]["agents"]` inside the project's `spec.trill.json` | The agent templates added to this dataflow. The analogue of the `dataflow.packages` lockfile. |
...
| `POST /api/agents/publications` `{coord}` | Publish | Publish an **owned, imported, store-backed** definition to the shared catalog (imported-only - rejects built-in/global/absent). |
...
**Each install materializes a per-project defaults record** (`spec.dataflow.agentDefaults`, memo `dev/23`): one independent `{revision, settings}` profile per project (seeded from the definition's `settingsDefaults` profile id/version; built-ins empty), preserved across canvas saves like the lockfile, dropped on uninstall, never reset by a reinstall. In the drawer, every *In dataflow* card carries a labeled **`Project agent settings`** cog opening the settings shell at the project scope, which edits this record (memo `dev/24`).
...
In the UI: **drag an added agent from the Agent Catalog palette onto a node** to attach it to that node, or **onto empty canvas** to attach it to the whole canvas (the drop target is resolved from the node under the cursor); node agents render as avatar badges on their node, canvas agents in the top dock bar. **Click a badge/tile** to open the **chat panel** (styled to the approved concept, `DEC-042`: one dark top header with ‹ › agent-cycling arrows that walk all attachments, the tinted bot + name + `idx / total`, the attached target + session chip, Clear conversation, and Close - no Pin; the intent renders as the conversation's first editable message, followed by dark user bubbles / avatar-prefixed agent rows and a pill input with a circular ↑ send). The **Agent Catalog drawer** header carries the Pin only (`DEC-042`) - pinning blocks the backdrop/Escape dismissals. Each send runs one turn through the run endpoint; the transcript is hydrated from the persisted session, so closing the panel (✕ / Escape - the agent stays attached), reopening, or reloading the page restores the conversation. The header also offers *Clear conversation* (confirm-first) and the ✕ on a badge detaches. A node target is validated against the *saved* project spec, so attach to a node after the project has been saved.

The **Browse all** scope is the 21 built-in agents ∪ any **published** definitions. Thirteen built-ins are the migrated prompt behaviors (Chat, Debug, Node Explainer, Dataflow Explainer, Connection Builder, the planners/validators, etc.), defined as a data-driven roster in [`app/agents/builtin.py`](../utk_curio/backend/app/agents/builtin.py), generated from the canonical prompt→agent map over `utk_curio/llm-prompts/*.txt` and validated through the manifest contract; the composites and specialists - **Node Builder** (`agent.node-builder`, memo dev/48), **Dataset Finder** (`agent.dataset-finder`, memo dev/50), **Dataflow Builder** (`agent.dataflow-builder`, memo dev/52), **Node Researcher** (`agent.node-researcher`, memo dev/67-4), and **Package Recommendation** (`agent.package-recommendation`, memo dev/84: identify/recommend node packages; installs surface as reviewed `package.install` proposals that apply through the existing package install dialog), the **Generated Content Evaluator** (`agent.generated-content-evaluator`, DEC-055 / memos dev/85-86: the advisory semantic-validation layer - findings with quoted evidence and a strictly findings-derived verdict; report-only, its judgment never approves anything and never replaces execution-based validation), the **Package Builder** (`agent.package-builder`, DEC-059 / memo dev/89: authors or extends node packages - templates, custom JS behavior, dependencies, assets - as ONE reviewed `package.draft.apply` draft built by the isolated build service; the promotion coordinator installs only the exact reviewed artifact digest; never installs or publishes itself), and the **Researcher** (`agent.researcher`, DEC-060 / memo dev/90: answers questions through the policy-gated web tools and composes the findings into post-it style note nodes; its reuse ladder has three rungs in order (DEC-062 / memo dev/93) - **reuse** a template this project can already instantiate, else **enlist** one the user has installed but this project has not declared, through a reviewed `package.install`, else **author** by delegating to the Package Builder with the post-it recipe's requirements; distinct from Node Researcher, the verification agent) - carry net-new instructions in the same directory. A built-in resolves for Import/Install straight from the roster (no prior store copy needed).
...
**The template vocabulary agents plan in (memo dev/93).** Every roster an agent sees is composed per run by the packages domain - the agents module owns no template knowledge - and there is exactly ONE of it, in ONE spelling, under ONE heading. Two rules make that hold:

- **One canonical id, three accepted spellings.** The canonical node type is the UNVERSIONED `<packageId>/<templateId>` that `available_templates` returns and a saved spec pins. But the surrounding system hands models the other two constantly: the client node registry keys descriptors VERSIONED (`<packageId>/<templateId>@<major>`, so the canvas graph and the runtime's own proposal previews speak it), and legacy trill files carry pre-package ENUM names (`DATA_LOADING`). So `packages.services.canonical_template_id` is the single home for that conversion, `resolve_template` is the single availability gate behind both plans and `node.create`, and a plan's node types are canonicalised where the plan is first PARSED - which is why the stored proposal, its pinned shape digest, and both apply-time re-checks agree by construction. A model quoting an id the system itself produced is never refused for the spelling.
- **Two buckets, because "missing" and "not enlisted here" are different.** A run that can add a template sees *Available node templates* (what this project can instantiate right now) and, when it holds `package.install`, *Installed but NOT enlisted in this project* (what the user already owns, named with the package to propose). Before this the roster had one bucket, so an installed notes package looked identical to no notes package at all, and an agent told to reuse concluded nothing existed and authored a duplicate.

`_ROSTER_GRANTS` decides who gets the roster: `node.create`, `dataflow.plan.write`, `package.install`, `package.draft.apply` - anything that can put a template on the canvas, plan one, enlist a package providing one, or author a new one. An authoring agent is included deliberately: not seeing what already exists is how duplicates get written.

**And the refusals must be self-correcting too (memo dev/105, DEC-067).** The ladder was populated correctly and still placed nothing once: the model quoted the package's bare manifest id, the `package.install` mint exact-matched the dirName and pointed at `packages.catalog` - a tool the Researcher does not hold - and the two millisecond refusals spent the three-round budget before the AUTHOR rung could be reached. Three rules follow. A mint accepts **every spelling its own roster taught** (`_resolve_catalog_dir_name`: dirName, bare id, or template id through `canonical_template_id`; an ambiguous major refuses naming every candidate; the canonical dirName is what gets pinned). A refusal's hint **names only sources the refusing run can read** (grant-aware: the *Installed but NOT enlisted* list always, `packages.catalog` only when granted). And a **parameter refusal is free of the round budget**: `ParamRefusal` marks refusals the model can fix by changing params, both tool loops admit `MAX_REFUSED_ROUNDS = 2` of them without spending a round (broken-store refusals stay unmarked so a dead store cannot loop), and the execution record carries `refusedRounds`. The Researcher's prompt matches: a REFUSED request is a correction to act on in the same run, a FAILED build is a stop, and a refusal is never evidence that a package does not exist. The roster also tells a note-composing run when nothing listed renders a note (`presentation` row flag) rather than letting silence read as "no rule here".

**And a required specialist must be installed by the same click that installs its orchestrator (memo dev/106, DEC-068).** The Dataflow Builder's Solve is a *server* path that invokes `node.content.generate`; only the Node Content Builder declares it. Installing the Builder alone left every fresh project failing all plan nodes in 14 ms with no model call - and the runtime's own remedy, the reviewed `project.install` proposal `REQ-ORCH-001` mandates, was minted into the attachment mirror and then dropped: the Solve path discarded the part, and no surface rendered a mirror-only `project.install`, so the user saw six bare "failed" pills and Retry re-minted the same invisible proposal. Four rules follow. `requiresAgents` declares the hard subset of `delegatesTo` (validated as a subset). `install_in_project` resolves the closure BEFORE writing (409 naming the missing ids, nothing written), then installs root + closure in ONE spec write; `uninstall_from_project` refuses (409, dependents named) while an installed template requires the coord. The catalog card discloses `requiresAgents` and the Add button reads "Add to dataflow (+N required)". And the Solve turn carries the proposal it mints plus one batch reason line; the builder strip offers Install/Dismiss from the mirror and shows each distinct failure reason once - the migration path for every project installed before dev/106. Import into *My imports* is deliberately NOT chained: adding materializes the definition; import is the ownership/publishing step.

**And the delegate half (memo dev/94).** The roster above rides a run's *grants*, and a delegated child holds none - DEC-046 makes it structurally tool-less. That is deliberate and unchanged, but it had a consequence nobody had traced: the Package Builder's instruction opened with "before authoring anything, read `packages.catalog`", a tool it holds only as a direct attachment, while the delegate path is the one packages are actually authored on. Its first rule was unexecutable and it had no way to know, so it authored - twice in one run, duplicating a package the user already had.

The rule that follows is worth stating as a design constraint, not just a fix: **an instruction must be executable on every path its agent runs on.** Where a tool-less child's instruction depends on facts, the runtime supplies them as *inputs* - the standing pattern, now applied four times: deterministic validator results for `research.verify` (dev/67-4), the build-request contract for authoring delegates (dev/90 A8), `existingPackages` - the packages the user already has, each with its `dirName`, whether this project has enlisted it, and its canonical template ids so `mode: "extend"` names something real (dev/94) - and the `research.notes.compose` trio (dev/95, delivering dev/90 Follow-up D): the runtime-run web search's bounded results, the installed presentation-template candidates, and the notes reply schema, so the Dataflow Builder can delegate a research question to a tool-less Researcher whose findings come back as reviewed, jointly-pending note proposals in the DFB's own chat. The payload bounds itself, because `delegation._frame_inputs` bounds nothing.

A doctrine also needs a reachable outcome. "Reuse first … never a duplicate package" was unanswerable while a draft was the only reply the runtime recognised, so the contract teaches a second legal reply (`insteadOfAuthoring` → `{"reuseExisting": {dirName, reason}}`), detected by schema and never by reading intent out of prose. It mints nothing, spends no correction rounds, and reports **success** - a run that produced the answer has not failed - and the hand-back tells the parent whether the package needs enlisting first. Note what this does NOT do: it makes reuse sayable and its consequence correct; it cannot enforce non-duplication, because "already satisfies the need" is a judgment, which is why an agent is making it.

**Dataset Finder (the second composite).** Attached to a data-loading node (other node kinds refuse with the requirement named - `compatibleTargets[].requires`) or the canvas for mission-first discovery, it proposes ranked dataset candidates in TWO lanes (docs/06): **External sources** (metadata suggestions - never a claim that a connector, credential, or availability exists) and **From your Data Catalog**, grounded exclusively in `catalog.search` results over the real catalog listing. Rows are informational, keyboard-operable multi-select - no per-row buttons; selection composes an editable confirmation prompt into the chat input. Confirming a catalog pick mints a reviewed `dataset.install` proposal whose apply runs the EXISTING dataset-only install flow (duplicates idempotent; a dataset gone between mint and apply → 409 + stale); confirming an external pick hands off to the user's own Node Builder attachment via a suggested prompt (DEC-047 - a depth-1 child can never mint proposals), and a missing Node Builder yields the reviewed `Install in project` proposal. No dataset pick ever installs an agent, and Dataset Finder never authors fetch code.

**Dataflow Builder (the third composite).** Attached to the canvas, it turns a goal - optionally seeded by a planning template - into ONE reviewable, **additive** plan proposal: a connected graph of typed placeholder nodes ("code pending", intent on each node), every type registry-validated, pinned against the current graph shape. Applying inserts the whole graph into the saved spec and the live canvas atomically; existing work is never removed or replaced. The builder strip in its chat shows the phase (Plan · Review · Solve · Ready): **Solve** - one authenticated click - fills pending plan nodes through bounded-concurrency depth-1 specialist runs (per-node digest guards mean user-edited nodes are skipped, never overwritten; failures isolate with subset Retry; a missing specialist yields one reviewed `Install in project` proposal), and **Run workflow** triggers the canvas's existing play-all. The orchestration session persists on the attachment, so a reload restores phase and progress. Large designs chain across successive additive plans - plan bounds are abuse backstops, not product ceilings (DEC-048). Replans may also **remove or rewire** on your explicit request (DEC-049): removals are reviewed by name - the card lists every node to be removed with a content warning and the edge cascade - and each victim is pinned by its content at proposal time, so editing a node after the plan was proposed makes the apply refuse instead of deleting your work. Plan edges may reference existing nodes, so extensions connect straight into the current graph.
...
**Publish is imported-only** (`DEC-030`): only an owned, imported, store-backed definition can be published - a built-in (or any global/absent) coordinate is rejected. Publishing copies the definition into a deployment-shared catalog at `.curio/agents-catalog/<id>@<version>/` ([`app/agents/publications.py`](../utk_curio/backend/app/agents/publications.py)), where every user's Browse all tab then lists it. A My imports card carries `publishable` (owned + store-backed) and `published` flags, so the drawer's Publish pill only appears for eligible definitions. **Publish is user-reachable**: upload-import (memo `dev/36`, the drawer's `Import agent` button) creates owned `imported`-trust definitions, which are exactly what Publish accepts.
...
- [`docs/AGENT-CATALOG.md`](AGENT-CATALOG.md) - the Agent Catalog: surfaces, storage layers, and the action matrix.
- [`docs/NODE-CATALOG.md`](NODE-CATALOG.md) - the Node Catalog, whose package/catalog model hookable agents mirror.
