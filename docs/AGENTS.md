...
| `POST /api/agents/projects/<projectId>/attachments/<attachmentId>/run` `{message}` | Run | Run one turn of the attached agent (system turn = **preamble + intent/instruction**, per the legacy composition + bounded prior session context + your message) via the provider port; returns `{reply}` and persists both turns to the session. |
