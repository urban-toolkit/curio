...
The rule that follows is worth stating as a design constraint, not just a fix: **an instruction must be executable on every path its agent runs on.** Where a tool-less child's instruction depends on facts, the runtime supplies them as *inputs* — the standing pattern, now applied three times: deterministic validator results for `research.verify` (dev/67-4), the build-request contract for authoring delegates (dev/90 A8), and now `existingPackages` — the packages the user already has, each with its `dirName`, whether this project has enlisted it, and its canonical template ids so `mode: "extend"` names something real. The payload bounds itself, because `delegation._frame_inputs` bounds nothing.
