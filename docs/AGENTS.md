...
| `POST /api/agents/projects/<projectId>/attachments/<attachmentId>/run` `{message}` | Run | Run one turn of the attached agent (system turn = **preamble + intent/instruction**, per the legacy composition + bounded prior session context + your message) via the provider port; returns `{reply, executionId, usage}` and persists both turns to the session. |
| `POST /api/agents/projects/<projectId>/attachments/<attachmentId>/run/stream` `{message}` | Run | Same turn, streamed as Server-Sent Events: `event: execution` `{executionId}` → `event: delta` chunks → `event: done` `{reply, executionId, usage}` (or `event: error`). Additive envelope — old clients skip unknown event names. Validation errors return plain JSON statuses before streaming; persistence matches the blocking run. |
