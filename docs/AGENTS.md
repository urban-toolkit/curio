...
| `POST /api/agents/projects/<projectId>/attachments/<attachmentId>/run` `{message}` | Run | Run one turn of the attached agent (system turn = **preamble + intent/instruction + the structured-tail instruction**, per the legacy composition + bounded prior session context + your message) via the provider port; returns `{reply, executionId, usage, content}` and persists both turns to the session. |
| `POST /api/agents/projects/<projectId>/attachments/<attachmentId>/run/stream` `{message}` | Run | Same turn, streamed as Server-Sent Events: `event: execution` `{executionId}` → `event: delta` chunks → `event: content` `{parts}` (when the reply carried a valid structured tail) → `event: done` `{reply, executionId, usage, content}` (or `event: error`). Additive envelope — old clients skip unknown event names. Validation errors return plain JSON statuses before streaming; persistence matches the blocking run. |
...
**Every run leaves an execution record** (memo `dev/37`, the first v2-maturation tranche): the agent turn persisted to the session carries `{executionId, pins, usage, durationMs, status}` — the transcript **is** the run history (memo `12`); there is no parallel execution store. The `pins` are the `DEC-031` reproducibility inputs resolved from what actually dispatched: coord, the manifest's instruction-prompt sha256 (null tolerated for unstamped built-ins), whether the intent was edited, provider type + model (never secrets), and the flat effective-policy snapshot admitted for the run. `usage` is the provider-reported **Actual** token count, or `null` when the provider reports none — never an estimate (memo `11`); the port's `usage_out` sink captures it per backend (openai `include_usage`, anthropic response/stream usage, gemini `usage_metadata`). Error turns carry `status: "error"`. The daily quota window also accumulates advisory Actual-token counters (`usage_today`), folded in post-run for every provider call **including** the recordless dev/25 title call — so the counters may exceed the transcript's sum, by design. `GET /api/agents/settings` and the project-defaults GET expose them as `usageToday`; the Quotas screen shows `N runs · X in / Y out tokens today (actual)` and the Cost screen's Actual line names tokens as available and USD as pending the tranche-3 price table. Old turns/windows without the new keys read clean — no migrations, and provider context ignores the extra keys.

**The settings screens** (`AgentSettingsModal`): the shared shell with the three v1 policy screens — Cost (estimated spend labeled as such; the Actual line shows real tokens today, with USD pending the tranche-3 price table), Quotas (runs + actual tokens today), Resource policies — at the **Account policy** scope (the labeled `Agent settings` cog in the roster header) and the **Project agent default** scope (the installed-card cog). Every field shows its effective value + source; edits are tighten-only (server-validated), revisioned (409 on stale), and the project scope offers *Reset to agent default*. No Publish/Release/Share exists in any settings scope.
...
**Tool contracts** (`app/agents/tools.py`, `DEC-017`/`DEC-043`): manifest `tools` entries — `[{id, required?}]`, capability-id grammar, no duplicates — are untrusted *declarations*, never grants. The server resolves `granted = requested ∩ registry ∩ policy` per run, where the typed registry currently ships **empty** (the first contract lands with its first consumer) and policy grants `read`-effect contracts only — a `mutate` contract is ungrantable until the review-before-apply flow exists. Granted ids are pinned on the execution record (`pins.tools`); a `required` tool that resolves no grant refuses the run with a 422 before admission (no quota consumed).
