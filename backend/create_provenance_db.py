...
commands = [
    ...
    "CREATE TABLE activity (activity_id integer primary key, workflow_id integer, ve_id integer, activity_name varchar(50), input_relation_id integer, output_relation_id integer, code TEXT, CONSTRAINT fk_act_wf FOREIGN KEY (workflow_id) REFERENCES workflow(workflow_id), CONSTRAINT fk_act_ve FOREIGN KEY (ve_id) REFERENCES versionedElement(ve_id), CONSTRAINT fk_act_rel_in FOREIGN KEY (input_relation_id) REFERENCES relation(relation_id), CONSTRAINT fk_act_rel_out FOREIGN KEY (output_relation_id) REFERENCES relation(relation_id));",
