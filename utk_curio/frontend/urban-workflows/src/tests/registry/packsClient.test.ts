/**
 * Regression guard for `registerPackKinds` (the conversion path from manifest
 ...
 *
 * The fixture is a hand-written subset of `packs/curio.builtin@1/manifest.json`
...
import { registerPackKinds } from '../../registry/packsClient';
import { clearPackNodes } from '../../registry/nodeRegistry';

const FIXTURE_PACK = {
  packId: 'curio.builtin',
  major: 1,
  version: '1.0.0',
...
describe('registerPackKinds → NodeDescriptor', () => {
  beforeEach(() => clearPackNodes());

  test('data-loading: code editor, upload icon, N output cardinality, no badge for built-in', () => {
    const [dl] = registerPackKinds([FIXTURE_PACK]);
  ...
  test('vis-vega: bidirectional handle, badge="VEGA", grammarId="vega-lite", hasProvenance=true', () => {
    const [, vega] = registerPackKinds([FIXTURE_PACK]);
  ...
  test('merge-flow: editor=null (none), 50x180 container, custom icon', () => {
    const [, , merge] = registerPackKinds([FIXTURE_PACK]);
