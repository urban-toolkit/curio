...
function NodeProvenance({ data, nodeType, setCode }: NodeProvenanceProps) {
    const { provenanceGraphNodesRef } = useProvenanceContext();
    ...
    useEffect(() => {
        if (provenanceBypass.current) {
            ...
            let activity_name = nodeType + "-" + data.nodeId;

            if (provenanceGraphNodesRef.current[workflow_name] == undefined)
                return;
            if (
                provenanceGraphNodesRef.current[workflow_name][activity_name] ==
                undefined
            )
                return;
            ...
            for (
                ...
                let obj =
                    provenanceGraphNodesRef.current[workflow_name][
                        activity_name
                    ][i];
    ...
    const onNodeClick = (nodeData: any) => {
        ...
        setCode(
            provenanceGraphNodesRef.current[workflow_name][activity_name][index]
                .code
        );
    };
