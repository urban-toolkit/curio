...
export function useCode(): IUseCode {
  ...
  const interactionsCallback = useCallback(
    (interactions: any, nodeId: string) => {
      setInteractions((prevInteractions: IInteraction[]) => {
        ...
        if (newNode) newInteractions.push({ nodeId: nodeId, details: interactions, priority: 1 });
  ...
  const loadTrill = (trill: any, suggestionType?: string) => {
    ...
    for (const node of trill.dataflow.nodes) {
      ...
      if (node.goal !== undefined) nodeMeta.goal = node.goal;
      if (node.in !== undefined) nodeMeta.inType = node.in;
      if (node.out !== undefined) nodeMeta.out = node.out;
      if (node.warnings !== undefined) nodeMeta.warnings = node.warnings;
      if (node.metadata !== undefined && node.metadata.keywords !== undefined)
        nodeMeta.keywords = node.metadata.keywords;
      if (suggestionType !== undefined) nodeMeta.suggestionType = suggestionType;
    ...
    for (const edge of trill.dataflow.edges) {
      ...
      if (suggestionType !== undefined) add_edge.data.suggestionType = suggestionType;
  ...
  const generateCodeNode = useCallback(
    (boxType: string, options: CreateCodeNodeOptions = {}) => {
      const {
        nodeId = uuid(),
        code = undefined,
