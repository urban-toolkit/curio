 ...
 *
 *   - **Versioned**:   ``<packId>/<kindId>@<major>``
 *   - **Unversioned**: ``<packId>/<kindId>``
 *
 * Unversioned refs (the default form used in trill files and the
 * pre-installed builtin pack) resolve to the latest installed major via
...
/** `it.vendor.pack/foo-kind@3` → `{ unversioned: 'it.vendor.pack/foo-kind', major: 3 }`. */
...
/** True if *nodeType* matches the unversioned `<packId>/<kindId>` shape (no `@major`). */
