...
export const useDataExportLifecycle: NodeLifecycleHook = (data, nodeState) => {
  ...
  const sendCodeOverride = async () => {
    nodeState.setOutput({ code: 'exec', content: '', outputType: downloadFormat });
    await downloadData();
    nodeState.setOutput({ code: 'success', content: 'Download complete.', outputType: downloadFormat });
  };
  ...
  useEffect(() => {
    nodeState.setOutput({ code: 'success', content: '', outputType: downloadFormat });
