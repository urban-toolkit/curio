...
class CatalogMutations:
    ...
    def delete_dataset(self, dataset_id: str) -> dict[str, Any]:
        ...
        for usage in usages:
            ...
            # Runs inside the section writer's lock (dev/82); ``None`` when the
            # dataflow holds no ref for this dataset skips the write. ``_hit``
            # binds per iteration so the flag can't leak across dataflows.
            def _drop_everywhere(
                refs: list[dict[str, Any]], _hit: list[str] = hit
            ...
            self.installed.mutate_refs(df_id, _drop_everywhere)
            if hit:
