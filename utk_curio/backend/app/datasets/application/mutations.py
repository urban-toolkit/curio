...
class CatalogMutations:
    ...
    def uninstall_dataset(self, dataflow_id: str, dataset_id: str) -> dict[str, Any]:
        ...
        # For computed datasets, remove the folder from the user's dataset store
        # so the dataset is fully gone (it can be regenerated by re-running the
        # node). Do this BEFORE rewriting the spec refs: the project save carries
        # forward computed refs whose dataset dir still exists, so the dir must
        # already be gone or the uninstalled ref would be re-added on the next save.
