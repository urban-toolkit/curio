...
and the seeder would happily re-install the pack the user just
removed (the regression behind the "I can't uninstall packs" UX bug).

This module owns a single ``<user>/packs/.seed-state.json`` file that
remembers, per pack dir name, whether the seeder has put a copy in
place and whether the user has explicitly uninstalled it. Both records
carry the ``fixtureMtime`` they were observed at, so:

* a pack the user uninstalled stays uninstalled across hot-reloads,
...
on a per-pack record are preserved on rewrite, and a corrupt or
...
class PackSeedRecord:
    """What the seeder remembers about one pack for one user."""
    ...
    @classmethod
    def from_json(cls, raw: object) -> "PackSeedRecord":
...
def _state_path(user_key: str) -> Path:
    return user_packs_dir(user_key) / STATE_FILENAME


def load(user_key: str) -> dict[str, PackSeedRecord]:
    """Return ``{ dir_name: PackSeedRecord }`` for ``user_key``.
    ...
    try:
        ...
        log.warning("Could not read pack seed-state %s: %s", path, exc)
        return {}
    if not isinstance(raw, dict):
        return {}
    packs = raw.get("packs")
    if not isinstance(packs, dict):
        return {}
    out: dict[str, PackSeedRecord] = {}
    for name, record in packs.items():
        if isinstance(name, str):
            out[name] = PackSeedRecord.from_json(record)
    return out
...
def save(user_key: str, records: dict[str, PackSeedRecord]) -> None:
...
def mark_seeded(user_key: str, dir_name: str, fixture_mtime: float) -> None:
    ...
    records[dir_name] = PackSeedRecord(
        seeded_at=time.time(),
        uninstalled_at=None,
...
def mark_uninstalled(user_key: str, dir_name: str) -> None:
    ...
    """
    records = load(user_key)
    prev = records.get(dir_name) or PackSeedRecord()
    records[dir_name] = PackSeedRecord(
        seeded_at=None,
    ...
    * runtime exists + fixture mtime unchanged → ``skip`` (idempotent).
    ...
    if not runtime_exists:
        if rec.is_uninstalled:
            ...
            # to respect the uninstall. The ``CURIO_RESEED_PACKS`` env
