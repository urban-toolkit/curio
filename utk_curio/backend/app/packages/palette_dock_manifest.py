...
from utk_curio.backend.app.packages.manifest import ManifestError, PackageLineageCoord, load_packageage_manifest
from utk_curio.backend.app.packages.storage import package_dir, user_packageages_dir


def merge_manifest_palette_dock_fork_hidden(package_dir_path: Path, hidden: bool) -> None:
    ...
    still validates as a package manifest.
    """
    ...
    load_packageage_manifest(package_dir_path)
    from utk_curio.backend.app.packages.installer import refresh_packageage_integrity  # noqa: PLC0415

    refresh_packageage_integrity(package_dir_path)
...
def _lineage_coord_dir(coord: PackageLineageCoord) -> str:
    return f"{coord.package_id}@{coord.major}"
...
def set_all_fork_parents_palette_visibility(user_key: str, *, visible: bool) -> None:
    ...
    for dir_name in sorted(parents):
        try:
            path = package_dir(user_key, dir_name)
        except OSError:
            continue
