"""Tests for :mod:`utk_curio.backend.app.packs.resolver`.
...
def test_merge_python_deps_conflict():
    ...
    packs_in_conflict = {p for (p, _r) in conflict.ranges}
    assert packs_in_conflict == {"alpha@1", "beta@1"}
...
# ---------------------------------------------------------------------------
# Pack DAG + project-level resolve
# ---------------------------------------------------------------------------
...
def test_resolve_for_project_pulls_transitive(tmp_curio, install_pack, manifest_dict):
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.base", major=1,
...
def test_resolve_for_project_missing_pack_dep(tmp_curio, install_pack, manifest_dict):
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.leaf", major=1,
            python_deps={},
            pack_deps={"ai.test.missing": "^1.0"},
            kinds=[_kind("leaf-kind")],
        ),
        sources={"leaf-kind": {"Default.py": "def run():\n    return {}\n"}},
    )
...
def test_resolve_for_project_detects_cycle(tmp_curio, install_pack, manifest_dict):
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.a", major=1,
            python_deps={},
            pack_deps={"ai.test.b": "^1.0"},
            kinds=[_kind("a-kind")],
        ),
    ...
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.b", major=1,
            python_deps={},
            pack_deps={"ai.test.a": "^1.0"},
            kinds=[_kind("b-kind")],
        ),
...
def test_resolve_for_project_duplicate_pack_id_requires_at_major(
    tmp_curio, install_pack, manifest_dict,
):
    kind = _kind("k1")
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.shared", major=1, kinds=[kind],
        ),
    ...
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.shared", major=2, kinds=[kind],
        ),
        sources={"k1": {"Default.py": "def run():\n    return {}\n"}},
    )
    install_pack(
...
def test_resolve_for_project_pack_dep_at_major(tmp_curio, install_pack, manifest_dict):
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.pick", major=1, kinds=[_kind("a")],
        ),
    ...
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.pick", major=2, kinds=[_kind("b")],
        ),
...
def test_lockfile_to_dict(tmp_curio, install_pack, manifest_dict):
    install_pack(
        "guest",
        manifest=manifest_dict(
            pack_id="ai.test.demo", major=1,
            python_deps={"numpy": "^1.26"},
        ),
    )
