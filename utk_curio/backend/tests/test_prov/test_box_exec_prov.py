...
class TestBoxExecProv:
    ...
    def test_creates_activity_execution(self, client, seeded_box, prov_db):
        wf, activity = seeded_box
        client.post("/boxExecProv", json=_exec_payload(wf, activity))
    ...
    def test_source_code_stored(self, client, seeded_box, prov_db):
        wf, activity = seeded_box
        code = "result = df.sum()"
        client.post("/boxExecProv", json=_exec_payload(wf, activity, source_code=code))
    ...
    def test_multiple_executions_increment(self, client, seeded_box, prov_db):
        wf, activity = seeded_box
        client.post("/boxExecProv", json=_exec_payload(wf, activity))
        client.post("/boxExecProv", json=_exec_payload(wf, activity, source_code="y = 2"))
