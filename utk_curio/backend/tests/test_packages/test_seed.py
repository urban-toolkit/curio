...
def test_second_run_is_a_no_op_when_nothing_changed(tmp_curio, real_fixtures_root):
    ...
    # Built-in is always reseeded (forced-builtin), but third-party packages
...
def test_uninstall_writes_tombstone_state(tmp_curio, real_fixtures_root):
    ...
    assert uninstall_packageage("guest", "ai.urbanlab.uhvi@1") is True
...
def test_repeated_seeds_after_uninstall_stay_no_op(tmp_curio, real_fixtures_root):
    ...
    uninstall_packageage("guest", "ai.urbanlab.uhvi@1")
    for _ in range(3):
        assert "ai.urbanlab.uhvi@1" not in seed_dev_packageages(user_key="guest")
    assert "ai.urbanlab.uhvi@1" not in _installed_names()
...
def test_fixture_mtime_advance_overrides_tombstone(tmp_curio, real_fixtures_root, monkeypatch):
    ...
    records["ai.urbanlab.uhvi@1"] = seed_state.PackageSeedRecord(
        seeded_at=None,
...
def test_curio_reseed_packages_env_overrides_tombstone(tmp_curio, real_fixtures_root, monkeypatch):
    seed_dev_packageages(user_key="guest")
    uninstall_packageage("guest", "ai.urbanlab.uhvi@1")
    assert "ai.urbanlab.uhvi@1" not in _installed_names()

    monkeypatch.setenv("CURIO_RESEED_PACKAGES", "1")
    seeded = seed_dev_packageages(user_key="guest")
    assert "ai.urbanlab.uhvi@1" in seeded
    assert "ai.urbanlab.uhvi@1" in _installed_names()
...
def test_reinstall_clears_tombstone(tmp_curio, real_fixtures_root):
    ...
    seeder would still treat the package as 'user-uninstalled' on the next
    ...
    install_packageage_from_directory("guest", src)
...
def test_untracked_existing_copy_is_adopted_into_state(tmp_curio, real_fixtures_root):
    """A runtime package with no seed-state record (because the user is
