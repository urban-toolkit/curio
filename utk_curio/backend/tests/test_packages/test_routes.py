...
def test_fork_install_sets_parent_palette_dock_hidden_and_toggle_round_trip(client, user_and_token, tmp_curio, install_packageage, manifest_dict):
    from utk_curio.backend.app.projects.services import _user_dir_key

    user, token = user_and_token
    uk = _user_dir_key(user)
    install_packageage(uk, manifest=manifest_dict(package_id="ai.palette.parentdock"))
    lineage = {
        "forkedFrom": {"packageId": "ai.palette.parentdock", "major": 1},
        "root": {"packageId": "ai.palette.parentdock", "major": 1},
    }
    install_packageage(
        uk,
        manifest=manifest_dict(package_id="ai.palette.forkdock", lineage=lineage),
    )

    packages = client.get("/api/packages", headers=_auth(token)).get_json()["packages"]
    parent = next(p for p in packages if p["packageId"] == "ai.palette.parentdock")
    assert parent["paletteDock"]["hiddenFromForkPaletteDock"] is True

    rev = client.post(
        "/api/packages/palette-dock/fork-parents",
        data=json.dumps({"visible": True}),
        headers=_auth(token),
    )
    assert rev.status_code == 204

    shown = client.get("/api/packages", headers=_auth(token)).get_json()["packages"]
    parent_shown = next(p for p in shown if p["packageId"] == "ai.palette.parentdock")
    ...
    hidden_again = client.get("/api/packages", headers=_auth(token)).get_json()["packages"]
    parent_hidden = next(p for p in hidden_again if p["packageId"] == "ai.palette.parentdock")
...
def test_packageage_palette_dock_visible_per_install(client, user_and_token, tmp_curio, install_packageage, manifest_dict):
    from utk_curio.backend.app.projects.services import _user_dir_key

    user, token = user_and_token
    uk = _user_dir_key(user)
    install_packageage(uk, manifest=manifest_dict(package_id="ai.palette.onesie"))

    vis = client.post(
        "/api/packages/ai.palette.onesie@1/palette-dock-visible",
        data=json.dumps({"visible": False}),
        headers=_auth(token),
    )
    ...
    packages = client.get("/api/packages", headers=_auth(token)).get_json()["packages"]
    row = next(p for p in packages if p["packageId"] == "ai.palette.onesie")
    ...
    packs2 = client.get("/api/packages", headers=_auth(token)).get_json()["packages"]
    row2 = next(p for p in packs2 if p["packageId"] == "ai.palette.onesie")
