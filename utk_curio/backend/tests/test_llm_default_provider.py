...
import utk_curio.backend.app.api.routes as routes
...
class TestResolveLlmConfig:
    def test_unconfigured_user_falls_back_to_default(self, monkeypatch):
        monkeypatch.setattr(routes, "DEFAULT_LLM_API_KEY", "seed-key")
        monkeypatch.setattr(routes, "DEFAULT_LLM_API_TYPE", "openai_compatible")
        monkeypatch.setattr(routes, "DEFAULT_LLM_BASE_URL", "https://sage200.evl.uic.edu/v1")
        monkeypatch.setattr(routes, "DEFAULT_LLM_MODEL", "llama4-nim")
        with _ctx():
    ...
    def test_guest_uses_guest_config(self, monkeypatch):
        monkeypatch.setattr(routes, "GUEST_LLM_API_KEY", "gkey")
        monkeypatch.setattr(routes, "GUEST_LLM_API_TYPE", "openai_compatible")
        monkeypatch.setattr(routes, "GUEST_LLM_BASE_URL", "https://sage200.evl.uic.edu/v1")
        monkeypatch.setattr(routes, "GUEST_LLM_MODEL", "llama4-nim")
        with _ctx():
    ...
    def test_guest_without_key_aborts(self, monkeypatch):
        monkeypatch.setattr(routes, "GUEST_LLM_API_KEY", "")
        with _ctx():
            g.user = _fake_user(is_guest=True)
