# `ai.urbanlab.uhvi@1` — fixture pack

A minimal pack used to exercise the Curio nodes-warehouse integration end-to-end
...
The pack is **self-contained** — every template preset lives under
...
## Seeding into a developer's pack store

The backend ships a small dev-only seeder
(`utk_curio.backend.app.packs.seed_dev_packs`) that copies this fixture
into `.curio/users/guest/packs/ai.urbanlab.uhvi@1/` on backend startup in
...
`CURIO_RESEED_PACKS=1` to force a refresh.
