...
- `POST /api/testing/reset-db` - truncate the mutable tables (`exec_cache_entry`, `project`, `auth_attempt`, `user_session`, `user`) and delete the per-user stores under `.curio/test/` that the freed ids would otherwise be handed (`stores: false` keeps them, to inspect what a test left). A request may name a subset of tables; anything outside that set is refused.
