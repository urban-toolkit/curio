...
export default function GrammarEditor({
	...
	useEffect(() => {
		...
		if(readOnlyBypass.current){
			let textarea = document.querySelector("#vega-editor_"+nodeId+" textarea") as HTMLTextAreaElement;
	...
	return (
		<React.Fragment>
			<div id={"vega-editor_"+nodeId} className="my-editor nowheel nodrag" style={{ overflowY: "auto", fontSize: "24px", height: "100%" }}>
				<JSONEditorReact
					nodeId={nodeId}
					content={grammar}
					mode={mode}
					modes={modes}
					onChangeText={(grammarObj: string) => {updateGrammarContent(grammarObj, readOnly)}}
