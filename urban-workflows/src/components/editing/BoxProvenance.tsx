...
function BoxProvenance({ data, boxType, setCode }: BoxProvenanceProps) {
    ...
    const [provEdges, setProvEdges] = useState<{id: string, source: string, target: string, label: string}[]>([]);
	...
	useEffect(() => {
        if(provenanceBypass.current){
            ...
            if(provenanceGraphBoxesRef.current[workflow_name][activity_name] == undefined)
                return;

            let newNodes = [];
            let newEdges = [];

            for(let i = 0; i < provenanceGraphBoxesRef.current[workflow_name][activity_name].length; i++){
                let obj = provenanceGraphBoxesRef.current[workflow_name][activity_name][i];

                newNodes.push({
                    id: 'n-'+i,
                    label: "\n Inputs: "+obj.inputs.join(", ")+"\n Ouputs: "+obj.outputs.join(", ")+"\n Code: "+obj.code.slice(0,15)
                });
    ...
    const onNodeClick = (nodeData: any) => {
        ...
        setCode(provenanceGraphBoxesRef.current[workflow_name][activity_name][index].code);
    }

    return (
        <div  className={"nowheel nodrag"} style={{width: "100%", height: "100%", position: "relative", marginLeft: "auto", marginRight: "auto"}}>
            <GraphCanvas
                nodes={provNodes}
                edges={provEdges}
