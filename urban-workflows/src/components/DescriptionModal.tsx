...
function DescriptionModal({
    ...
    const getTypeDescription = (boxType: BoxType) => {
        ...
        if(ConnectionValidator._inputTypesSupported[boxType].length > 0)
            linesText.push("Supported input types: "+ConnectionValidator._inputTypesSupported[boxType].join(", "));
        ...
        let purposeMap: any = {
            [BoxType.DATA_LOADING]: "The Data Loading box is responsible for getting data from the outside world into the workflow.",
            [BoxType.DATA_EXPORT]: "The Export box is responsible for getting data from the workflow to the outside world.",
            [BoxType.DATA_CLEANING]: "The Data Cleaning box is reponsible for performing cleaning operations on the data.",
            [BoxType.DATA_TRANSFORMATION]: "The Data Transformation box is responsible for performing any kinds of transformations to the data.",
            [BoxType.COMPUTATION_ANALYSIS]: "The Computation Analysis box is the box generic box responsible for performing any kinds of computations.",
            [BoxType.FLOW_SWITCH]: "The Flow Switch box is responsible for choosing which incoming data flow will be passed forward to the next box",
            [BoxType.VIS_UTK]: "The Urban Toolkit box is responsible for visualizing geolocated data.",
            [BoxType.VIS_VEGA]: "The Vega box is responsible for visualizing 2D plots.",
            [BoxType.VIS_TABLE]: "The Table box is responsible for displaying DataFrames and GeoDataFrames in a tabular format.",
            [BoxType.VIS_TEXT]: "The Text box is responsible for displaying text.",
            [BoxType.VIS_IMAGE]: "The Image box is responsible for displaying images.",
            [BoxType.CONSTANTS]: "The Constant box stores a constant.",
            [BoxType.DATA_POOL]: "The Data Pool is reponsible for storing data that can be interacted by all connected visualizations. Interactions can also be propagated to other Data Pools."
        };
        ...
        if(ConnectionValidator._outputTypesSupported[boxType].length > 0)
            linesText.push("Supported output types: "+ConnectionValidator._outputTypesSupported[boxType].join(", "));
        ...
        <>
            <Modal show={show} onHide={closeModal}>
                ...
                <Modal.Body>
                    ...
                    {custom != undefined && custom ? <p>Custom template: {name}</p> : custom != undefined && !custom ? <p>Default template: {name}</p> : null}
                    {description != undefined ? <p>{description}</p> : null}
                    {accessLevel != undefined ? <p>Access Level: {accessLevel}</p> : null}
                    {getTypeDescription(boxType).map((line: string, index:number) => {
