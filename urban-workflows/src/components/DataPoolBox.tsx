...
function DataPoolBox({ data, isConnectable }) {
        ...
        if(newInput != ""){
            ...
            if(parsedInput.dataType == "dataframe"){
                ...
                if(parsedInput.data.interacted == undefined){
                    ...
                    for(const dfIndex of dfIndices){ // initializing the interacted attribute
                ...
                if(data.propagation != undefined){
                    ...
                    let propagatedIndices = Object.keys(data.propagation).map((index: string) => {return parseInt(index)});
                ...
                for(const feature of parsedInput.data.features){ // initializing the interacted attribute
        ...
        if(output.content != ""){
            ...
            for(const interaction of data.interactions){
                ...
                for(const select of selects){
                    ...
                    if(details[select].source == BoxType.VIS_UTK){ // interactions from UTK only affect matching named geodataframes
                    ...
                    if(details[select].type == VisInteractionType.POINT){ // solve point interaction
                        localInteractedIndices.push({
                            priority: details[select].priority, 
                            indices: details[select].data.map((index: number) => {return index})
                        });
                    }else if(details[select].type == VisInteractionType.INTERVAL){ // solve interval (brushing) interaction
                        let brushedColumns = Object.keys(details[select].data);

                        let interactedObj: {priority: number, indices: number[]} = {
                            priority: details[select].priority,
                            indices: []
                        };
                        ...
                        for(let i = 0; i < objectsCounter; i++){
                            ...
                            for(const brushedColumn of brushedColumns){
                                ...
                                if(brushBoundaries.length > 0 && typeof brushBoundaries[0] == 'string'){ // categorial or ordinal variable
                                ...
                                }else if(brushBoundaries.length == 2){ // numerical interval
                ...
                if(plotResolutionMode == ResolutionType.OVERWRITE){
                    for(const elem of localInteractedIndices){ // using the interactions of the plot with higher priority
                        if(elem.priority == 1){
                            interactedList = [...elem.indices];
                        }
                    }
                }else if(plotResolutionMode == ResolutionType.MERGE_AND){ 
                    let allArrays = localInteractedIndices.map((elem: any) => {return [...elem.indices]});
    
                    if(allArrays.length > 0)
                        interactedList = allArrays.reduce((a: number[], b: number[]) => a.filter(c => b.includes(c))); // index is only include if it was interacted in all plots
            ...
            if(resolutionMode == ResolutionType.OVERWRITE){
                for(const elem of interactedIndices){ // using the interactions of the plot with higher priority
                    if(elem.priority == 1){
                        interactedList = [...elem.indices];
                    }
                }
            }else if(resolutionMode == ResolutionType.MERGE_AND){ 
                let allArrays = interactedIndices.map((elem: any) => {return [...elem.indices]});
            ...
            let propagationObj: IPropagation = {nodeId: data.nodeId, propagation: {}};

            let objectsCounter = 0;

            let buildingsLayer = false;

            if(parsedInput.data.features != undefined && parsedInput.data.features.length > 0 && parsedInput.data.features[0].properties.building_id != undefined)
                buildingsLayer = true;
    ...
    useEffect(() => {
        const selectBetween = document.getElementById(data.nodeId+"_"+"select_between") as HTMLElement;
    ...
    const ContentComponent = ({ outputTable, data }:{outputTable: any, data: any}) => {
        return (
                                        ...
                                        {Object.keys(row).map((column, columnIndex) => {
                                            return (
                                                <TableCell
                                                    ...
                                                    align="right"
                                                >
                                                    {row[column] != undefined && row[column] != null ? shortenString(row[column].toString()) : "null"}
                                                </TableCell>
                                            );
                                        })}
                                    </TableRow>
        ...
        <>
            ...
            <BoxContainer nodeId={data.nodeId} data={data} templateData={templateData} setOutputCallback={setOutput} promptDescription={promptDescription}>
                ...
                <BoxEditor
                    ...
                    contentComponent={<ContentComponent outputTable={outputTable} data={data} />}
