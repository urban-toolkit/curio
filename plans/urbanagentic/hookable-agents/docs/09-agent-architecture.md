...
- **Recoverable, not replayed.** Current streamed foreground runs persist execution records and
  cancellation state and never silently replay provider/tool work. *(dev/115, `DEC-073`: in the
  single-process topology a Solve is a detached job that outlives the request; a session left
  `solving` by a dead process is reconciled to `interrupted` on read and Retry is a linked
  execution.)* Multi-instance scheduling and a durable lease owner remain gated by `OQ-009`.
...
receives provider secrets.
  *(dev/116, `DEC-074`, 2026-09-08: node-level credentials joined this posture — a data-loading node reaches an API key only as `curio_secret("<name>")`, resolved server-side per run and injected into the sandbox namespace as a callable; agents receive names and `use` lines, never values; the per-user store is 0600 plaintext behind a `ConnectionKeyStore` interface that the T4 encrypted store replaces without touching callers.)*
