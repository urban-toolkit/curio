...
**Status: APPROVED — IN PROGRESS (2026-09-09). Commit 1 landed on `imp/agentcatalog` as `1a957ead`: `workflow_spec.is_executable_kind` (the ONE predicate), `run_through_node` refuses a non-code target by name (`notExecutable`), `validate_candidate` verdict `not-executable`, the per-node Solve answers without a round or a sandbox call (card kind result, text *Not executable: … Play the dataflow to see it*), validate-node proposes the generation labeled and the plan ledger proceeds as for a pass, the batch's `_record_outcome` writes such content with `verification.status = "not-executable"` (reachable after commit 2). Tests: runner (+3), validation (+1), rounds loop (+1), per-node Solve (+1), validate-node route (+1); backend `test_agents` + `test_execution` + `test_backend` 1183 passed / 2 skipped. **Finding:** the built-in `spatial-join` template (engine python, editor none) is absent from `workflow_spec.NAMESPACED_TO_LEGACY`, so the runner classifies it *passive* — it now reads *not executable* rather than falsely verified; mapping it (and any package Python kind) is follow-up F5. Deviation: `nodeStates[ref]` reads `validated` for a not-executable plan node (the chip vocabulary is the frontend's, commit 5 decides a word). Commit 2 landed as `fdeedfca`: `_solve_waves` (depth over the batch's own targets, data-flow edges only, cycle → one last wave), the wave loop with a `solve_wave` event, `_apply_contents` shared by `_persist_wave` and `_finish` (spec lock, deleted/user-edit guards, `persisted` set so a landed content is never re-guarded), per-wave `nodeRuns` + `solvingSince` refresh, the re-read spec for the next wave's loop and context, `upstreamOutputs` (`_upstream_outputs_for`) on the legacy inputs and the loop's `extra_inputs` + the prompt clause, the gate `if verify and _is_executable(node)`, `verification.status = "not-executable"` on legacy-written browser kinds. Pins flipped: `test_pass_writes_only_after_the_code_ran` (3 sandbox payloads: loader; loader re-run in the stats slice with the PERSISTED code; stats), `TestSolveSourceGrounding` (the sibling now runs in wave 2; the retry count), `TestProposeModeSolve` (an executed review), `TestSolve._applied_plan` stubs the sandbox. New tests: wave order on the wire + `upstreamOutputs` in the first generation and the correction, per-wave persistence observed while wave 2 is gated, `_solve_waves` (diamond / non-target upstream / Interaction edge / cycle). Backend 1189 passed / 2 skipped; frontend attach suites 277 (the wave event is ignored until commit 5 renders it). Commit 3 landed as `2387a570`: `DEFAULT_SOLVE_BATCH_DEADLINE_S` = 45 min, `solve_batch_deadline_s()` (`CURIO_SOLVE_BATCH_DEADLINE`, the `exec_timeout_s` pattern), `_batch_deadline_spent` checked before every wave and every node dispatch → undispatched targets `pending` with the reason (a `node_result` each), `batch_reason` set once, `notAttempted` lists them, phase `applied`; a `precondition` verdict (slice bound, cycle) → `skipped` with the bound named, not corrected (the loop breaks after one round) and never `failed`; pending/skipped card rows carry their reason; the stale-marker semantics documented in code. Deviation from §2: a slice-bound skip lands as `nodeRuns = "skipped"` (the existing skipped semantics — retrying the same slice re-hits the same bound; the runner's own message says to run the dataflow manually). Tests: deadline after wave 1 (deterministic through a counting `_batch_deadline_spent`), slice bound → skipped in one round, the env reader. Backend 1195 passed / 2 skipped. Commit 4 landed as `29225ca2`: `run_through_node(prior_outputs=)` seeds and skips passed ancestors (`reused`, never the target), `validate_candidate(prior_outputs=)` with `evidence.reusedNodes` + `evidence.output` (the artifact record), the loop's `prior_outputs_fn` resolved per round with a one-time silent retry without reuse when the target's input load names a vanished artifact (`_looks_like_a_vanished_reused_input`, marker heuristic recorded in code), `attempt.reusedNodes` / `attempt.reuseRetried`, the batch records `output` on `wave_outputs` and hands the resolver to the loop; `upstreamOutputs` strips the artifact record. The pass pin now shows 2 sandbox payloads (the loader; the stats node fed by the loader's artifact). Tests: runner (+2), validation (+1), loop (+3: reuse recorded, vanished-artifact retry, a real failure stays the candidate's). Backend 1201 passed / 2 skipped. Deviation from §2: the retry trigger is the TARGET's input-load failure — a vanished artifact fails the node that reads it, not the reused node, so the memo's 'blocker is a reused node' wording named the wrong node. Commit 5 (frontend + docs + ledgers) pending.**
