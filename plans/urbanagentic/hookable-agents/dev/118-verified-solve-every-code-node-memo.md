...
- **F5** `spatial-join` and package-provided Python kinds are absent from `NAMESPACED_TO_LEGACY`, so the runner treats them as passive (never executed by validation or the e2e runner) — map them so verification reaches them. **Re-scoped by dev/119 (2026-09-09): the premise was half wrong — `spatial-join` has no code (a backend endpoint does its work) and is right to read not executable; the real defects are versioned ids (live: the per-node Solve refuses palette-dragged nodes), `data-summary`, and the package Python kinds; the fix is registry-driven executability, not a longer list.** **CLOSED by dev/119 (2026-09-09, `DEC-076`, commits `c43eedaa`/`33308182`/`92901dda`): executability is read from the roster; `spatial-join` reads *no code the sandbox could run*.**

## Live re-test (2026-09-09, gemma4, `curio start` stack, the dev/115 A3 Task 2 prompt through the HTTP API)

**Pass 1** — plan of 10 nodes; Solve ran 5 waves. Wave 1: five data-loading nodes — one solved (the neighborhoods loader through `curio_dataset_path`), three declined honestly (`source-missing`: no catalog dataset for population / economy / vulnerability), one refused after three rounds (`ungrounded-source`: inline data). Wave 2: merge-flow written `not-executable`. Wave 3: the analysis node reused the solved loader's output (`reused 1`) — and **passed on round 2 after failing round 1 with `'NoneType' object has no attribute 'merge'`**: its slice had executed the four empty loaders as seed-only bodies, handed `None` downstream, and gemma4 coded around `None`. A hollow pass on inputs that do not exist. Waves 4–5: data-pool, autk-grammar, vis-vega written `not-executable`. **Harness gap → fixture** (`71445ad2`): for validation runs (`strict_upstream`) an upstream code node with no content is a blocker by name before anything runs (`report.upstreamEmpty`); the loop stops after one round without a correction; the batch leaves the dependent **`pending` with `waiting — upstream node … has no content yet — solve or fill it first`** so Retry runs it once the upstream is solved; the per-node Solve says the same. A plain Run (dev/71) keeps Play's semantics. The `TestSolveSourceGrounding` sibling pin, which had asserted the sibling "solved" against an empty refused loader, was the same hollow pass in test form — it now asserts the wait.

**Pass 2** — plan of 10 nodes, 4 waves: wave 1 three loaders (one solved, one declined, one refused); wave 2 merge-flow `not-executable`; wave 3 the join node **`pending · waiting — upstream node 'bcf6…' has no content yet`** with the solved loader reused and nothing run; wave 4 five browser-rendered nodes (`merge-flow`, `data-pool`, two `vis-vega`, `vis-simple`) written `not-executable`; phase `applied` (Retry targets the loaders and the waiting join). Acceptance criteria 1, 4, 8 met live (criterion 1's chart is written *not executable*; there was no upstream for a verified second wave because the loaders' sources do not exist in this catalog — the wait is the honest outcome); criteria 2–3 and 9 are covered by the deterministic mirrors.
