...
| DEC-007 | **Retired by `DEC-048`** — LangChain was the initially approved runtime behind a stable application boundary; the shipped orchestration is direct provider-port code, and the LangChain seam reopens only with `DEC-021` background execution. | `09-agent-architecture.md` |
...
| DEC-021 | Execution uses persisted leases/heartbeats. Expired nonterminal work becomes `interrupted`; provider/tool calls are never replayed automatically, and retry creates a linked execution. **Gate note:** `DEC-021` proper (leases/heartbeats, background execution — the `DEC-048` LangChain re-open condition) remains open with `OQ-009`; `DEC-050` delivered only its user-facing streamed-Solve slice. | `10-agent-plan-decision-closure-and-hardening-memo.md` |
