...
- A shared settings dialog shell with dedicated Cost, Quotas, Resource policies, Prompt quality, Prompt editor, and Prompt audit screens opened by clear authorized cog/settings controls. *(Shipped: the Cost/Quotas/Resource-policy screens at all scopes — dev/24/42; the Prompt quality/editor/audit screens remain v2 under `DEC-058`.)*
...
- Framework-neutral runtime contracts with a LangChain adapter for the first implementation. *(Superseded by `DEC-048`: the shipped runtime is direct provider-port code; a LangChain adapter reopens only with `DEC-021` background execution.)*
...
- In-place mutation of installed/published prompt bytes or silent substitution of the `DEC-055` authored generated-content evaluator.
...
| DEC-007 | **Retired by `DEC-048`** — LangChain was the initially approved runtime behind a stable application boundary; the shipped orchestration is direct provider-port code, and the LangChain seam reopens only with `DEC-021` background execution. *(dev/115, `DEC-073`, 2026-09-08: the single-process slice is DELIVERED — detached re-attachable Solve jobs, `interrupted` reconciliation on read, linked Retry; multi-instance ownership stays `OQ-009`; the LangChain re-open condition was met and declined with recorded criteria.)* | `09-agent-architecture.md` |
...
| DEC-021 | Execution uses persisted leases/heartbeats. Expired nonterminal work becomes `interrupted`; provider/tool calls are never replayed automatically, and retry creates a linked execution. **Gate note:** `DEC-021` proper (leases/heartbeats, background execution — the `DEC-048` LangChain re-open condition) remains open with `OQ-009`; `DEC-050` delivered only its user-facing streamed-Solve slice. *(dev/115, `DEC-073`, 2026-09-08: the single-process slice is DELIVERED — detached re-attachable Solve jobs, `interrupted` reconciliation on read, linked Retry; multi-instance ownership stays `OQ-009`; the LangChain re-open condition was met and declined with recorded criteria.)* | `10-agent-plan-decision-closure-and-hardening-memo.md` |
| DEC-022 | Provider credentials live behind account provider profiles and opaque secret references. Remote egress is explicit, custom endpoints are SSRF-checked, and local failure never falls back to a remote provider. *(dev/116, `DEC-074`, 2026-09-08: node-level API keys now follow the same shape — a name in code, a value only server-side and in the sandbox namespace; the encrypted secret store is still the T4 remainder and lands by swapping `ConnectionKeyStore`.)* | `10-agent-plan-decision-closure-and-hardening-memo.md` |
...
| DEC-051 | Input topology is a **parsed, plumbed, three-layer-enforced arity model** (dev/67-3): manifest `inputPorts[].cardinality` parses once (`parse_cardinality` → min/max) and `available_templates` serves per-port rows + `maxIncomingEdges` — where declaration and rendering disagree, the RENDERED capacity is the enforceable truth (one edge per rendered handle, handles 1:1 with ports; merge-flow's 5 slots are the sole multi-edge surface; declared `[1,n]` maxima are aspirational — the input plumbing is scalar per handle). Enforcement: (1) plan MINT refuses invalid fan-in (net incoming = plan edges + dev/59-surviving existing edges) with a corrective error naming the Merge resolution — invalid topology is unmintable, never repaired after execution; (2) plan APPLY writes explicit `sourceHandle`/`targetHandle` (plan edges may name merge slots via `toHandle`; unnamed take the lowest free slot; the bridge passes them through — ending the merge-dead-until-reload bug); (3) `onConnect` refuses a second edge into an occupied non-merge handle with the Merge-suggestion toast (per-handle, so spatial-join's named handles work; merge byte-identical; the LOAD path warns but never drops persisted edges). Unknown/out-of-scope templates fail open. | `67-3-topology-model-merge-resolution-memo.md` |
| DEC-053 | Agent egress is **one policy-gated chokepoint** and external discovery is **verified, provider-agnostically** (dev/67-4): `app/agents/egress.py` is the only module that speaks HTTP for agents — https/http only, private/loopback/link-local/reserved addresses refused AFTER DNS resolution (SSRF shapes incl. rebind), redirects re-checked per hop (≤5), 256KB body cap, ≤4 web calls per run, auditable per fetch. `web.fetch`/`web.search` are ordinary read contracts (search is deployment-configured via `CURIO_SEARCH_URL`; absent → an honest "not configured" error). **The Socrata hallucination fix generalizes to ANY dataset API connection** (owner directive): `verify.py`'s GENERIC endpoint probe is the universal gate; provider refinements (Socrata today) are registry entries adding richer evidence — never new gates. Every external Dataset Finder row carries a deterministic `verification` verdict (verified/unreachable/refused/loudly-unverified) probed by the RUNTIME, never claimed by the model; `agent.node-researcher` (17th roster entry, `research.verify`/`research.summarize`) is chainable from the builder agents — and because DEC-046 children are structurally tool-less, research.verify delegates receive runtime-verified evidence in their inputs and synthesize over it. LangChain: declined again for the ≤3-step research chain; "unbounded research chains" joins DEC-021 as a monitored re-open condition. | `67-4-node-researcher-verified-discovery-memo.md` |
| DEC-059 | **Package authoring is a NEW specialist over an isolated build service — never an extension of Package Recommendation** (dev/89, owner-approved 2026-08-19): `agent.package-builder` (20th built-in; `package.build`/`package.extend`/`node.kind.author`; callers send the `package.create-or-extend` intent) owns the package artifact; recommendation stays catalog-grounded discovery + reviewed install. ONE mutate contract — `package.draft.apply` — runs the build service: typed content-addressed requests (input digest = idempotent build identity), restricted ephemeral workspaces (scrubbed env, rlimits, process-group kills, sanitized diagnostics), registry-only SRI-verified JS resolution with SBOM/policy gates (no registry configured = a blocking finding, never an ambient npm fallback), the deployment-pinned deterministic compiler with host-global externals (a second React copy is impossible by construction), the CSP-sandboxed five-state preview that GATES Apply, deterministic packaging + full provenance, and a promotion coordinator that is the ONLY install authority (verify-on-read exact digest, 409 stale/collision protection, backup held until activation completes, journaled disconnect-safe repeated Apply, honest `rolled-back` vs `rollback-failed`). Appearance is INSTANCE identity: one shared node-appearance utility per layer (6-name palette + six-digit hex, AA-refusing normalization, derived ink) with the canonical `metadata.appearance.backgroundColor` round-trip through save/load/bridge. Follow-ups: A (Curio package backend sandbox), B (activation/restart lifecycle), C (`node.appearance.write` agent recolor). | `../dev/89-agent-package-builder-capabilities-memo.md` |
| DEC-067 | **A proposal refusal must be self-correcting on the path that produced it: it accepts every spelling its own roster taught, it names only sources the refusing agent can read, and a parameter refusal is free of the round budget** (dev/105, owner-approved 2026-08-25; extends DEC-063 from prompts to refusal texts and loop accounting). Field evidence: the Researcher's 2026-08-25 run — dev/93's ladder was correctly populated (`curio.notes@1` offered on the ENLIST rung with its dirName, `package.install` granted) and still placed nothing, because (D1) the model quoted the bare manifest id, `_mint_package_install` exact-matched the dirName and refused pointing at `packages.catalog`, a tool the Researcher does not hold; (D2) `MAX_TOOL_ROUNDS = 3` counted the two millisecond refusals as rounds, so the AUTHOR rung was structurally unreachable; (D3) the prompt's one "if a proposal fails, stop" sentence taught the give-up, and the model asserted "not in the catalog" (false). Posture: ONE resolver (`_resolve_catalog_dir_name`) accepts dirName / bare id / template id (through `canonical_template_id`, never a second parser), refuses an ambiguous major naming every candidate, and pins the CANONICAL dirName; the miss hint branches on the run's grants; `ParamRefusal` (a `str` subclass — every consumer sees a string) marks refusals the model can fix by changing params, and both tool loops admit `MAX_REFUSED_ROUNDS = 2` of them without spending a round (broken store/catalog/spec refusals stay unmarked — they cost real work and must keep counting so a dead store cannot loop), recorded additively as `execution.refusedRounds`; the prompt distinguishes a REFUSED request (correct or take the next rung, same run) from a FAILED build (stop), forbids absence claims the refusal did not make, and forbids narrating request mechanics; the roster tells a `research.notes.compose` run when nothing listed renders a note (additive `presentation` row flag) and states an empty roster instead of omitting it. Scope deliberately narrow: the marker is applied at the two mints the field run hit; widening is one `_refuse_params(...)` per site when a lane shows the same failure. | `../dev/105-researcher-reuse-ladder-unreachable-rungs-memo.md` |
| DEC-069 | **A publication carries a publisher record (`publication.json` beside the byte-identical artifact); publish-over, unpublish, and the delete-import guard authorize against that record — never against what the caller's own store happens to contain. A coordinate published by another account is not overwritable (409: publish under a new version). Legacy record-less entries fall back to the dev/107 heuristic and are claimed on the owner's next publish; the backfill tool writes a record only on an exact single match and never guesses.** (dev/110, owner-approved 2026-08-27; hardens DEC-030's imported-only publish and makes DEC-057/058's "owner unpublishes" enforceable.) |
| DEC-068 | **An agent's server-invoked dependencies are declared (`requiresAgents` ⊆ `delegatesTo`) and installed by the user's own Install click as one atomic closure; the orchestrator still never installs silently, and a minted install remedy must be visible where the failure is** (dev/106, owner-approved 2026-08-25; refines DEC-046/`REQ-ORCH-001` — which binds the *agent*, not the user's explicit install). Field evidence: the Dataflow Builder's 2026-08-25 run — DFB + Researcher installed, Solve failed all six plan nodes in 14 ms with `tools: []` (no provider call): `delegation.resolve("node.content.generate")` → not-installed (only `agent.node-content-builder` declares it); `_mint_project_install` stored the reviewed proposal in the attachment mirror, the Solve path discarded the part (D1), no surface rendered a mirror-only `project.install` (D2), the per-node `error` was dropped on persist and by the SSE handler (D3), and `install_in_project` never read `delegatesTo` (D4). Posture: `requiresAgents` validated as a subset of `delegatesTo` (self/duplicate/non-id rejected); `delegation.required_closure` (transitive, `find_visible`, cycle-safe, depth ≤ 8) and `required_by`; install refuses-before-write (409 naming the missing ids) then materializes and records root + closure in ONE spec write, response `{agents, installed, required}`; uninstall 409 while a dependent needs the coord (dev/101 shape, no cascade); cards carry server-resolved `requiresAgents` rows; the Solve turn's content is `[card, proposal part]` with ONE `reason:` line and `payload.reason`, and a refused mint is reported as refused, never as "awaits review"; the strip offers Install/Dismiss from the mirror and renders distinct `node_result.error` reasons once; a `project.install` apply repaints the palette. Only the DFB declares a requirement (NCB); every other delegate stays optional — no 15-agent fan-out. Import into My Imports is NOT chained (materialization satisfies "imported"; import is the publishing step). | `../dev/106-required-delegate-install-closure-memo.md` |
| DEC-070 | **Plan edges carry an explicit kind; the plan validator owns acyclicity over data edges at mint AND apply; a reviewed plan may be edge-only** (dev/112, owner-approved 2026-08-27; applies DEC-063 to edges — an intent the agent can state must be expressible on the path it runs on; complements DEC-051's "topology validated before anything materializes"). Field evidence: the owner's 2026-08-25 session (`78fbbe35…`, project `cb605bfd…`) — five rounds of "fix the cycle", each a correct diagnosis and a VALID applied plan `{nodes:[], edges:[vis→merge], removeEdges:[old]}` whose intended `Interaction` kind the grammar dropped and whose apply hardcoded `sourceHandle:"out"`, so the agent removed and recreated the same data edge (ids `2480b41b → 8fe92401 → 605b9549 → acdd7365 → a479f59d`); mint validated endpoints + fan-in only, the bulk bridge bypassed the canvas `hasCycle`, the runner's refusal was never shown to the agent; the grammar refused additive plans without nodes, so the model added filler note nodes "to make the plan valid"; the applied line said "removed 0 nodes" after an edge removal. Posture: `edges[].kind ∈ {data, interaction}` (data byte-absent; `type` accepted as the Trill alias); `plan_topology.py` is the ONE helper — NET graph (saved − removals − cascade + plan) over DATA edges, `closing_plan_edges` blames a plan only for cycles IT closes (a user-drawn cycle it leaves alone is reported, never refused), the preamble's interaction rule (visualization ↔ data-pool) executable with a corrective error naming the fix; apply re-checks BEFORE mutating (`_mark_stale` persists the spec) → 409 + stale, per-edge → refused row named; interaction edges materialize as the Trill shape (`type: Interaction`, `in/out` both ends, bidirectional on the canvas via one `appliedEdgeToCanvasEdge`); fan-in and merge slots ignore interaction edges; edge-only plans valid; removed connections reviewed by name (`removedEdges`) and counted in the applied line; every applied turn ends with `Topology: acyclic.` or `Topology: cycle through …` and the instruction requires reading it before claiming a repair. | `../dev/112-dataflow-plan-contract-hardening-memo.md` |
| DEC-071 | **A connection attachment targets an existing edge id; its one mutation is the reviewed `node.insert` — remove the edge, add the node between the endpoints, reconnect source → new → target keeping the merge slot, re-target the attachment to the downstream replacement — validated against the saved spec and refused before any write on drift** (dev/113, owner-approved 2026-08-27; delivers dev/15 §5's third hook, deferred at dev/48; consumes DEC-040/047/049/051/070). Posture: Node Builder alone declares `connection` (`targets=("canvas","node","connection")`, `tools += node.insert`); the edge id comes from the attachment record, never from params (least privilege); mint composes the equivalent plan and applies the dev/112 rules; an interaction connection (visualization ↔ data-pool) has no inside — a node would have to be a pool for one half and a visualization for the other — so inserting into one is refused plainly rather than half-supported; apply pins the edge digest (id, endpoints, handles, kind), checks template + endpoints, then mutates in one write and re-targets EVERY attachment on the removed edge to the downstream edge (deterministic: the agent sits where data enters the target); the applied turn carries the Topology verdict. Frontend: `pickEdgeAtPoint` (sampled bezier, node → edge → canvas order), a hover store for the drop affordance, `EdgeAgentBadges` via `EdgeLabelRenderer` in both edge components, server-resolved `targetLabel` on cards, `targetContext` composes edge + endpoint snapshots (content sizes, never content), and the bridge treats `appliedGraph` ahead of `createdNode` so the split paints atomically. Out of scope by owner instruction: the legacy handle-side suggestions and every `llmRequest` caller. | `../dev/113-connection-attach-target-memo.md` |
| DEC-072 | **Every agent-authored node content passes ONE runtime source-grounding gate before it can become a proposal or be written by Solve: a local path is grounded only when the user typed it or the Data Catalog resolved it (literal path or the portable `curio_dataset_path("<id>")`); a URL only when the runtime probed it this run or an already-verified candidate row carries it (401/403 = the endpoint exists, credential-gated); a data-loading node with no source only when synthetic data was explicitly requested and is labeled as such; anything else is refused with the correction named — never proposed, never written, never described as existing** (dev/114, owner-directed 2026-09-08; closes the builder half of the asymmetry dev/67-4 named — DEC-053 verified the finder's rows, nothing verified what Node Builder wrote; applies DEC-063 twice more and DEC-067's self-correcting refusals). Field evidence: issue #298 (`pd.read_csv("bras_ibge_data.csv")`, `bras_geosampa_boundary.shp` — FileNotFoundError on first run) and the owner's 2026-09-08 gemma4 test (4/5 links broken, Census fetch → HTTP 400): the deployment's models cannot vouch for a file, a URL, or a request — only the runtime can. Posture: `source_grounding.py` is the ONE gate (ast over a function-body parse, f-string prefixes, regex fallback; `catalog-id` refs; user paths from the user's own turns + the current message; URLs never grounded by user text); `_gate_generated_content` at node.create / node.content.write (dev/73 mint inherits) / node.template.create / Solve (batch base built in the request thread; propose mode gates before the mint); ONE per-run egress budget (`_run_egress_budget`) shared with candidate verification, per-URL cache, verified-row memory; `ParamRefusal` refusals with grant-aware hints; the sixth and seventh runtime-supplied-inputs applications (`dataset.discover`: catalog listing + #269 `CANDIDATES_INSTRUCTION`; `node.content.generate` on a data-loading node: `sourceGrounding` with the exact `use` line); `_mint_candidates_from_delegate` (schema-recognized, catalog rows dropped unless listed, DEC-053 verdicts, `_candidates_pending_review` refuses a same-turn create — the user confirms first); `catalog.search` rows carry `path` + `loader`; Node Builder `tools += catalog.search`, `delegatesTo += agent.dataset-finder`; the proposal `source` payload (display + provenance, never a pin) renders as the review card's Source block; the preamble's three bare-filename exemplars replaced. Out of scope (recorded): parameter validation of verified endpoints (F1), the legacy `llmRequest` callers (F2), provenance INTO the spec node (F3), package-authored templates (F4). | `../dev/114-data-loading-source-grounding-and-hallucination-memo.md` |
| DEC-073 | **Solve is the engineering loop for data-loading nodes, and it runs in the background: the user's explicit Solve (the Dataflow Builder's batch, or a per-node Solve from any agent attached to the node) authorizes a detached, re-attachable job that executes the node's code in the sandbox (Play's dataset-path mapping, a fetch-sized timeout), feeds every failure back to the content generator with the traceback, the previous attempt, the grounded sources and fresh probe evidence for the URLs it fetched, re-runs (≤2 corrections), and lands only code that passed — written directly into an empty node, or as a reviewed `node.content.write` that has already run for a node that had content; every attempt is recorded and shown as a collapsed trail; proposals are never executed at mint and Apply is never gated on execution. The job survives the request, re-attaches through `GET …/jobs/stream`, and a session left `solving` by an execution this process does not hold is reconciled to `interrupted` on first read, with Retry a NEW execution carrying `retryOf` — DEC-021 delivered for the documented single-process topology (liveness IS the lease; multi-instance stays `OQ-009`). The DEC-048 LangChain re-open condition is met and DECIDED: not adopted (Option A — direct code over `runner`/`validate_candidate`/the `build_jobs.py` shape), with LangGraph adoption criteria recorded (OQ-009 multi-instance; a second cross-restart orchestration needing reusable intermediate state; a branching correction graph).** (dev/115, owner-approved 2026-09-08 with Amendments A1–A3; consumes DEC-006/021/046/048/050/053/054/063/067/072.) Field evidence: the owner's gemma4 test (a Census fetch node answering HTTP 400 after passing the static gate — only a run catches a wrong request shape) and dev/67-0's standing requirement ("no shortcut for code considered trivial") that Solve's write mode had never adopted. Posture: `_verified_content_rounds` is the ONE loop (validate-node, Simulation Mode, Solve batch, per-node Solve; `start_from_current` runs the node's current code as round 0; the DEC-072 gate runs before every execution — a fabricated path is a failed round of kind `ungrounded-source` that never reaches the sandbox); the runner sends `dataset_paths` + `user_key` like `/processPythonCode` and treats a sandbox timeout as the node's failure (`ExecutionTimeout`), timeout `CURIO_VALIDATION_EXEC_TIMEOUT` (default 300 = the sandbox wall clock); everything needing the request context (grounding base, dataset-path mapping) is resolved in the request thread and handed to the job; `agent_jobs.py` holds one live job per attachment with per-user backpressure refused before any in-flight state persists; the Dataflow Builder alone declares `runtime.execution: "background"` (the dock's running dot); v1 verifies data-loading nodes only (`verify: false` keeps the legacy write). *(dev/118, `DEC-075`, 2026-09-09: widened to every executable kind, in waves — see DEC-075.)* Deviations recorded: no persisted heartbeat (racy against `_finish`; liveness is the lease in one process); per-node Solve has no cancel in v1; propose-mode Solve keeps dev/67-7's PASS-or-FAIL mint with the trail. | `../dev/115-verified-solve-background-execution-memo.md` |
| DEC-074 | **A data-loading node reaches a key-gated API through a named, per-user connection key — never a literal: the user saves the key once (masked, write-only, bound to a host, with how the API expects it: `code` / `query:<param>` / `header:<Name>`), node code refers to it only as `curio_secret("<name>")`, the backend resolves the names that appear in the code at execution time — Play and the verified Solve loop alike, per round — and hands the values to the sandbox inside the execution request, where they exist only as an injected callable in the node's namespace (never an environment variable — node code reads `os.environ`; never a staged file; never a log line; popped from the isolated child's request before the node runs). Agents learn a key's existence as a `use` line in their grounded inputs (`availableSecrets`) and never see, request or emit its value; the DEC-072 gate grounds a saved name as a `secret` ref, refuses an unknown name listing the saved ones, refuses a credential-shaped literal before anything runs, and hints at the saved key on a 401/403 endpoint; a Solve correction may probe the composed request WITH the key (query/header deliveries) — redacted, never cached, never a verified source; a credential decline ends the loop as `source-missing` with a concrete remedy (`connection-key` → Add key for <host>; `use-connection-key` → Solve again); every captured stdout/stderr is redacted at the sandbox boundary. Encryption at rest stays T4's remainder: `.curio/users/<key>/connection-keys.json` is 0600 plaintext (unreadable by isolated node code via `SENSITIVE_PATHS`) behind `ConnectionKeyStore`, the one class T4 swaps.** (dev/116, owner-approved 2026-09-08; consumes DEC-006/022/053/061/063/067/072/073.) Field evidence: dev/115 §A3.1 — the Census API redirects a key-less query to a "Missing Key" page and the loop could only name the missing key. Deviations recorded: routes on their own blueprint (`/api/users/me/connection-keys`); the sandbox's missing-key sentence does not list saved names (the gate does); the redactor lives in `utk_curio/common` (shared by backend and sandbox, which never import each other); the settings section is a collapsed `<details>` whose body mounts only while open; cards reach the modal through a window event answered on the dock overlay. | `../dev/116-data-loading-api-key-input-memo.md` |
| DEC-075 | **Solve verifies every executable node kind — in topological waves, persisted per wave, honest about what cannot run, bounded in time: a Solve batch groups its targets by depth over the plan's data-flow edges and runs wave by wave (roots first; a wave's targets in parallel under the worker pool); each wave's verified content is written at the wave boundary under the spec lock with the user-edit guard, `nodeRuns` updated and `solvingSince` refreshed — the boundary is the heartbeat, and a process that dies between waves keeps every persisted wave (DEC-021); the next wave's loop and context read the re-read spec, and a dependent's generation and corrections receive `upstreamOutputs` (the output data type its verified upstreams produced); an ancestor that passed earlier in the batch stands in by its recorded output (`prior_outputs`, never the target) and a vanished artifact costs one silent whole-slice retry, never a round. A node kind the sandbox cannot execute (grammar, data-pool, passive, unknown packages — `workflow_spec.is_executable_kind` is the ONE predicate) is never called verified: the runner refuses it by name before anything runs, the verdict is `not-executable`, the batch writes its content like the legacy path with `verification.status = "not-executable"`, the per-node Solve answers without a round, validate-node proposes the generation labeled. A batch has a wall-clock budget (`CURIO_SOLVE_BATCH_DEADLINE`, default 45 min) checked before every wave and every dispatch — what it did not reach stays `pending` with the reason and Retry continues; a slice-bound or cycle refusal (`precondition`) is `skipped` with the bound named after one round, never `failed`.** (dev/118, owner-approved 2026-09-09; supersedes DEC-073's v1 scope note "data-loading nodes only"; consumes DEC-021/050/054/067/072/073/074.) Evidence: dev/115 F1 was called "a one-line `verifyKinds` change" — the survey found the line drags four things: no ordering (all targets to three workers in plan-apply order), a single end-of-batch write against the batch-start spec (every downstream target would fail `upstream-blocker`), pass-through targets ending `ok: True` and failing open to `pass` (a LIVE hazard for the per-node Solve and validate-node), and no batch deadline while the runner re-executed the whole ancestor slice every round. Deviations recorded in `BL-P5-20260909-52`: a slice-bound skip lands as `nodeRuns = skipped` (retrying the same slice re-hits the bound); the vanished-artifact retry triggers on the TARGET's input-load failure (the memo's "blocker is a reused node" named the wrong node); a not-executable plan node reads `validated` in the plan ledger so Simulation Mode proceeds; `spatial-join` (python, editor none) is absent from `NAMESPACED_TO_LEGACY` and reads not-executable (F5). *(dev/119, `DEC-076`, 2026-09-09: the predicate's SOURCE OF TRUTH moved from the legacy name list to the template roster — `engine`/`hasCode`/no `backendHandler` — with the legacy tables as the offline fallback; F5's spatial-join premise was half wrong: it has no code and is right to read not executable.)* | `../dev/118-verified-solve-every-code-node-memo.md` |
| DEC-077 | **Agent quality is measured against the shipped examples by a DETERMINISTIC comparator over the production path, and the measurement may not weaken the expectation, gate a release, or grade itself** (dev/121, proposed 2026-09-09; consumes DEC-048 (plan → review/apply → Solve), DEC-062 (one vocabulary, one canonicaliser), DEC-072 (source grounding), DEC-073/075/076 (verified Solve, waves, executability from the template), DEC-055 (the evaluator has no authority), DEC-058 (Prompt Quality stays deferred — this is engineering test infrastructure, not that screen), DEC-063 (a schema-recognised decline is a success)): every shipped example — 11 curated and 20 legacy — has a reviewed, checked-in prompt fixture (`docs/schemas/example-prompt-fixture.v1.json`) pairing a natural-language prompt with the example's digest, its declared `dataflow.datasets`/`dataflow.packages`, its normalized expected graph, node intents, execution mode, a capability tier with JUSTIFIED skips, thresholds, a fine-tuning split and a review record. The evaluated model receives the prompt and nothing else (a leak test enforces it: no node or edge id, no canonical template id, no content line, no plan-grammar token, no JSON). Reconstruction runs the ordinary product path from an empty project and is scored SEMANTICALLY — canonical template ids and roles, topology with edge kinds and merge slots, declared dependency references, absence of invented templates/packages/datasets/paths/URLs, intents, and Solve's own verdicts read from `verification.status` (never the plan ledger's `validated`, which dev/118 F6 shows would over-credit) — with regenerated ids, layout, formatting and behaviourally equivalent code ignored by construction, and node order irrelevant because the canonical form is a canonical LABELING. Three prohibitions carry the decision: a construct the agent contract cannot express is a NAMED capability gap and the expected graph keeps it (eight fixtures need `interaction-edge` on `imp/agentcatalog`, where dev/112's `edges[].kind` is absent); a live-model run is an opt-in evaluation REPORT (`CURIO_EVAL_LIVE=1` plus `--live-eval`, never CI, `isReleaseGate: false` in the file, no invented cost); and no agent grades an agent — the comparator is deterministic code, so DEC-055's report-only boundary and DEC-028's self-certification firewall are untouched. The fixture format is the fine-tuning export unit with immutable train/validation/held-out splits, and the export refuses an unreviewed prompt or a training draw on the held-out or validation splits; there is no training verb and no AI Settings Model Training panel until its own memo settles provider capability detection, dataset consent/licensing, redaction, cost disclosure, job status/cancellation, trained-model versioning, the held-out evaluation gate, activation and rollback. Posture: implemented on `imp/agentcatalog` (commits `37533fce`…`d34f766c`); the browser tier needs a dedicated stack and the 31 prompts await owner review. Deviations recorded in `BL-P5-20260909-55`. | `../dev/121-example-derived-agent-validation-harness-memo.md` |
| DEC-076 | **Whether the sandbox can run a node kind is read from its template — `engine` ∈ {python, javascript}, an editable code surface (`hasCode`) and no `backendHandler` — through the template roster the agents already receive (`executable` on every roster row, `packages_services.template_is_executable` the ONE derivation), never from a hand-kept list of legacy names. Node type ids are normalised through the one canonicaliser (`@<major>` stripped) before any classification. The runner, `validate_candidate`, the verified loop, the Solve batch and the per-node Solve classify against one roster snapshot per batch/request (`roster_templates`); a template's engine routes the run (`/exec` vs `/execJs`); the legacy `NAMESPACED_TO_LEGACY`/`CODE_TYPES` tables remain only as the offline fallback when no roster is reachable (the e2e runner over a raw file) and a roster row known and not executable is never `code`, whatever the legacy name said. A template with no code — `spatial-join`, `merge-flow`, `data-pool`, the grammars — is `not-executable` with the honest reason: *it has no code the sandbox could run; it works in the browser or through its own service*. The frontend reads executability from the proposal (`pins.executable`, minted from the roster row) with the registry descriptor as the fallback for older parts, and keeps no list of its own.** (dev/119, owner-approved 2026-09-09; consumes DEC-061/062/072/073/075.) Evidence: a LIVE defect — palette-dragged nodes carry versioned ids (`curio.builtin/data-loading@1`) that `normalize_type` did not strip, so the runner classified them passive and the per-node Solve refused a user's real Data Loading code; `data-summary` (`DATA_SUMMARY`) was never in `CODE_TYPES`; seven `curio.weather@1` and three `ai.urbanlab.uhvi@1` Python code templates read passive; the dev/118 F5 premise (map `spatial-join`) was half wrong — its work is a frontend POST to `/spatial_join`. Deviations recorded in `BL-P5-20260909-53`: the roster snapshot is `{canonical_id: {executable, engine}}` — two fields, nothing more; a backend-handler template is not sandbox-executable (dev/91's path, F2 of dev/118 unchanged); `sandbox checkIOType` legacy-only matching and the `tests/test_frontend/workflow_spec.py` copy drift are recorded findings, not fixed here. *(dev/120, 2026-09-09: the legacy tables lost their two phantom kinds `CONSTANTS`/`FLOW_SWITCH`; every legacy code kind must now have a namespaced spelling — test-pinned; the sandbox refuses no node by its type name.)* | `../dev/119-executability-from-the-template-registry-memo.md` |
| DEC-066 | **Backend handler dependencies live in per-package overlays — the shared interpreter is touched only for warm-sandbox templates; the probe stays dep-blind by design and says so** (dev/97, owner-approved 2026-08-24; delivers the dev/91 §0.1 Option-2 hardening DEC-064 named as the resident-services prerequisite): ONE pip primitive (`install_python_deps_to_target`, `pip --target`, no skip logic — wipe-before-build makes a fresh dir the only input) builds `.curio/users/<key>/package-backend-overlays/<dir>/` at BOTH install authorities under ONE routing core with two manifest adapters (`_dep_route`: overlay for backend-bearing packages, both when python-engine `hasCode` templates coexist, host otherwise — typed for promote/catalog, raw for the dev/96 card, so the card can never disagree with the install), size-capped (`CURIO_BACKEND_OVERLAY_MAX_MB`, default 512) with promotion-rollback compensation; workers AUTO-resolve the overlay inside the dev/92 target lock (no half-built overlay is observable) and the explicit parameter stays the test seam. A POST-APPLY PROBE (one real worker, installed entry, real overlay on PYTHONPATH) gates activation with rollback — the mixed-tree shadowing edge dies at Apply, not at the user's first Run. dev/92's `restartRecommended` is deliberately NARROWED to the host portion (fresh workers cannot split-brain); the journal records `overlay {libs, bytes, reason}`. Probe posture = **Option A**: python stays review-only at build (Option B's hermetic pip-at-build rejected — PyPI network in the build phase, offline breakage), so handlers import declared deps LAZILY inside `handle()` — the rule taught in prompt AND buildRequestContract, and a module-level import of a declared dep refuses naming exactly that fix with -/_ name-drift tolerated (an undeclared module honestly gets no lazy advice). Uninstall finally keeps dev/91 §6.7's promise: `remove_backend_residue` sweeps overlay + data dir + pin at DELETE and prune, while the invocation LEDGER survives — retention owns its expiry, never uninstall. | `../dev/97-backend-overlay-isolation-memo.md` |
| DEC-065 | **dev/90 Follow-up D is DELIVERED — the Dataflow Builder delegates research to a tool-less Researcher whose findings return as reviewed note sequences; dev/89 Follow-up C (agent recolor) is DEFERRED demand-driven** (dev/95, owner-approved 2026-08-23): the wiring is composed ENTIRELY from standing patterns — `agent.researcher` joins the DFB's `delegatesTo` and the DFB gains the reviewed `node.create` lane (the dev/93-D4 additive-reviewed-lane posture); `_enriched_delegate_inputs` gains its FIFTH runtime-supplied-inputs application (DEC-063's list): ONE egress-policed `tools._execute_web_search` run by the RUNTIME (bounded 5×300, provider errors verbatim — the child must say search was unavailable, never invent), `presentation_templates` candidates (the A14 profile, installed+enlisted, enumerated once beside `available_templates_report`), and the server-owned `notesReplyContract` teaching the A13 row; the child's reply is recognized by SCHEMA only (`notes` must be a LIST — answer-only replies carry `[]`, chat JSON never matches) and `_mint_notes_from_delegate` extends the one-mint-policy to SEQUENCES: one `node.create` proposal per row through the ONE creation lane (dev/93 vocabulary, A12 bounds, dev/89 appearance normalization, A13 index-default colors), landing as a same-run A16 jointly-pending row applied in any order, grant-gated on the PARENT with every degradation named (no grant → failed with the answer still leading; empty notes → ok, the answer IS the product; no template → ok with 'notes skipped' pointing at the Researcher's own attachment, because depth-1 keeps the enlist/author ladder out of a delegate's reach). The DFB prompt's byte-pin updated ONCE, deliberately, with the edit it guarded against (02cad07d…→6000ae6c…, the dev/48 D4 rule). **Follow-up C's deferral**: both real color surfaces exist (creation-time agent color per A12/A13, the manual `NodeColorControl` since dev/89 commit 8) and no user ask, recorded scenario, or live transcript has requested agent-driven recolor of EXISTING nodes; re-open requires a real demand signal plus its own memo (dev/89's `node.appearance.write` digest-pinned design is the recorded starting point). A recorded decision, never a silent omission — the DEC-056/DEC-064 pattern. | `../dev/95-dataflow-builder-researcher-delegation-memo.md` |
| DEC-064 | **dev/89 Follow-up B is DELIVERED AS RE-SCOPED — the on-demand sandbox absorbed activation, and what remained ships as consistency, honesty, and quarantine; resident generated services are DESCOPED demand-driven** (dev/92, owner-approved 2026-08-21, implemented 2026-08-23): the §0 table shows dev/91's nothing-resident model structurally absorbed five of B's seven mandate items (start/hot-load, readiness, rollback, version routing, activating states); the code audit found the three that remained and dev/92 ships exactly those. **B-1 consistency**: ONE per-(user, target) lock — `packages/target_locks.py`, a thin wrapper over dev/93's `common/file_locks.keyed_thread_lock`, never a second keyed-lock implementation — now serializes THREE consumers (promote, rollback, and `invoke_handler`'s microseconds-long READ phase; workers always run outside it), so an invocation can no longer observe the installer's non-atomic rmtree+move window or a files-vs-pin mismatch (the pre-dev/92 transient 404/'reinstall'-during-the-reinstall refusals). **B-2 restart honesty**: pip's `InstallReport` is consumed at BOTH install authorities instead of discarded — a non-empty `installed` list becomes `restartRecommended {libs}` on the promotion journal (persisted, disconnect-safe), the apply payload + backend-owned result-turn text, and the catalog install response + drawer notice banner (one sentence composer, `packageRestartCopy.ts`, the retentionCopy pattern); skipped-only/idempotent runs stay silent, 'recommended' not 'required' because pip's own installed-list is the exact truth and only version changes split-brain the warm sandbox (dev/91 §0.1). **B-3 crash-loop quarantine**: 3 consecutive INFRASTRUCTURE failures (the ledger's own worker-*/no-reply/bad-reply taxonomy — one classification, two consumers; well-formed handler-error replies and refused payloads never count) quarantine the (user, package, handler) for 120s with a 503 naming the cooldown and the way out, gated before any worker slot is spent, reset by any success or a reinstall's new entry pin, half-open on expiry, every transition a ledger row; in-process by design (restart clears — the correct semantic). **The descope**: resident generated services (supervision, ports, hot-load, long-lived draining, readiness loops, restart backoff) have zero demand evidence — streetvision stays first-party by design — and re-open requires (a) a real resident-only need a user or recorded scenario brings, (b) the dev/91 §0.1 Option-2 dependency-overlay hardening landed first, (c) an owner-approved memo of its own; dev/91's refusal surfaces keep naming Follow-up B, which now points here. | `../dev/92-backend-activation-lifecycle-memo.md` |
| DEC-063 | **An instruction must be executable on EVERY path its agent runs on — a tool-less child gets its evidence as INPUTS, and every doctrine gets a reachable outcome** (dev/94, owner-approved 2026-08-21; amends DEC-046's evidence posture without touching its structure): DEC-046 children stay structurally tool-less, tail-less and cycle-free, but that is only sound if nothing in their instruction depends on a tool they cannot hold. The Package Builder's opening rule — "before authoring anything, read `packages.catalog`" — named a tool it holds ONLY as a direct attachment, while the delegate path is the one packages are actually authored on; the rule was unexecutable, the agent could not know that, and it authored a duplicate of a package the user already had. So: where a tool-less child's instruction depends on facts, the RUNTIME supplies them as inputs — the standing pattern, applied three times now (deterministic validator results for `research.verify`, dev/67-4; the build-request contract for authoring delegates, dev/90 A8; and `existingPackages` — dirName + `installedInProject` + canonical template ids so `mode: "extend"` names something real, dev/94) — each payload bounding itself, because `delegation._frame_inputs` bounds nothing but the child's reply. Second half of the rule: a doctrine with no reachable outcome is not a doctrine. "Reuse first … never a duplicate package" was unanswerable while a DRAFT was the only reply the runtime recognised, so the contract teaches a second legal reply (`insteadOfAuthoring` → `{"reuseExisting": {dirName, reason}}`), recognised by SCHEMA and never by reading intent out of prose; it mints nothing, spends no correction rounds, and reports SUCCESS — a run that produced the answer has not failed (DEC-062's honest-status rule is "produced NOTHING must not claim ok", which this satisfies) — while the hand-back tells the parent whether the package must be enlisted first. Deliberate non-goal, stated so it is never mistaken for a guarantee: this makes reuse SAYABLE and its consequence correct; it cannot ENFORCE non-duplication, because "already satisfies the need" is a judgment, which is why an agent is making it. | `../dev/94-authoring-delegate-reuse-evidence-memo.md` |
| DEC-062 | **One node-type vocabulary, one canonicaliser, one roster — and reuse-first gets a reachable middle rung** (dev/93, owner-approved 2026-08-21; supersedes DEC-052 in part — the `installedTemplates` frontend producer it introduced is RETIRED, because the server already appends the authoritative roster to the same run and the two contradicted each other): a node type has ONE canonical form (unversioned `<packageId>/<templateId>`) and THREE legal spellings the system itself emits (versioned, from the client registry / canvas graph / the runtime's own proposal previews; and legacy `ENUM_NAMES` from pre-package trill files), so `packages.services.canonical_template_id` is their single home, `resolve_template` is the single availability gate behind BOTH plans and `node.create` (`require_authorable` the one intended difference — a plan places a typed placeholder, a create writes content), and a plan canonicalises at the PARSE boundary so the stored proposal, its pinned shape digest, and both apply-time re-checks agree by construction; apply compares canonically but never rewrites the stored value, so in-flight proposals keep their digest. The run carries ONE roster (the client-composed duplicate is retired) in TWO buckets — *available here* and *installed but NOT enlisted here* — because those were indistinguishable, and an agent told to reuse an installed template it could not see authored a duplicate package instead; `agent.researcher` therefore gains the reviewed `package.install` lane as the ladder's middle rung (reuse → enlist → author), and `agent_catalog_overview` enumerates the user's store as well as the committed catalog so an agent-authored package can be enlisted into a second project at all. Supporting posture: the package store is seeded serialized + atomically (never rmtree-then-copy into the live path, never a per-request forced re-copy) and self-heals a broken built-in; an unreadable installed package is LOUD (`available_templates_report`) and refusals say "the store is degraded" instead of blaming the id; and the delegated package draft joins plans and generated node content in having capped correction rounds that feed the REAL error back to the same delegate, with the package id pinned across rounds (a rename is a failed correction, not a new package) and terminal refusals not retried. Standing lesson, now with two data points (dev/90 A14, dev/93 D3): when one value has several legal spellings in the same system, every validator must accept all of them — or the system manufactures refusals no model can fix. *(dev/120, 2026-09-09: two hand-kept second vocabularies retired — the sandbox's name-keyed `checkIOType` validators, a copy of the manifest's port types that no caller had reached since namespaced ids; and the e2e suite's full copy of `workflow_spec.py`, which a merge had put back in place of dev/67-7's shim — the shim is restored and identity is test-pinned.)* | `../dev/93-dataflow-plan-template-vocabulary-memo.md` |
| DEC-061 | **Generated server-side package code runs ONLY in the on-demand package backend sandbox — never in Curio's host process** (dev/91, owner-approved 2026-08-21; delivers dev/89 Follow-up A): a package declares `backend {entry, handlers[{name, timeoutClass}]}` gated on the reviewed `server-code` permission (`server-network` for network use), templates link Run via `backendHandler`, and each invocation spawns ONE short-lived `build_workspace` worker (scrubbed from-scratch env — no secrets exist to steal — rlimits, killpg, capped I/O) where the Curio-owned harness speaks the versioned `curio.pkgbackend.v1` envelope (ONE contract module; probes answered at load+resolution). Nothing is resident — upgrades are ordinary exact-digest promotes, deliberately narrowing Follow-up B to resident services/draining/version routing. Build-time gates: a static policy scan honest about what a regex can do (escape-hatch families blocked with file:line + fix; network per declared permission; `open()` deliberately unblocked — the runtime boundary is the jail) and a `probing` pipeline phase running every declared handler in a REAL sandbox worker, gating Apply exactly as preview gates looks. Both install authorities pin the entry digest; invocation verify-on-read 409s drift naming reinstall. Audit: one append-only ledger row per invocation (sizes/outcomes, never payloads) under the operator's `packageBackend.ledgerArchiveAfterDays` DEC-057 declaration. The §0.1 evidence (five pip paths into ONE shared interpreter, install-side upgrade blast radius, partial uninstall ref-counting, warm-sandbox staleness) grounds the venv decision: Option 3 — defer per-package envs/overlays, but the worker interpreter (`CURIO_BACKEND_SANDBOX_PYTHON`) and a PYTHONPATH overlay slot are invocation PARAMETERS, so overlay isolation lands later with zero contract change. | `../dev/91-package-backend-sandbox-memo.md` |
| DEC-060 | **Custom looks are PROMPT-DRIVEN — never first-party fixtures** (dev/90, owner-approved 2026-08-20; supersedes dev/89 commit 9 in part): the Package Builder's instruction carries the ONE generic authoring contract (behavior keys = the manifest's, hook `(data, nodeState) => { contentComponent }`, never-bundled host externals, React-elements-only rendering, the appearance contract, the preview sandbox named as enforcement) and NO scenario content; `agent.researcher` (21st built-in — "Researcher", the notes-scenario owner, disjoint by id/name/capability from `agent.node-researcher` verification) owns the post-it recipe, gathers findings itself through the policy-gated web tools (A1 — sources ride the notes as https links; unconfigured search reported honestly, never invented), and delegates authoring — the dev/73 one-mint-policy extends to delegate-returned drafts (a parseable child build request becomes a runtime-minted `package.draft.apply` proposal, gated on the PARENT's grant; draft-shaped chat prose never mints). The Dataflow Builder is byte-pinned untouched (sha256 test); Follow-up D wires its delegation later. Egress: the operator's `CURIO_SEARCH_URL` host is an exact host+port trusted exemption (A2 — schemes and cross-host redirect hops keep full default-deny) and the search adapter speaks the public keyed-GET provider shapes (A3 — SearXNG/SerpAPI/SearchApi/Google CSE/Brave-shaped; header-auth APIs honestly unsupported; LangChain declined again — not a dependency, one policy-gated GET needs no framework). | `../dev/90-prompt-driven-custom-node-looks-memo.md` |
| DEC-056 | **OQ-011 resolved**: the concept screens' `Validation` card is a **category view over the shipped family** — syntax (`code.syntax.analyze`), coherence (`workflow.coherence.validate`), outputs/data-types (dev/67-7 execute-through + the DEC-052 journal), external facts (`research.verify`), semantics (DEC-055's `content.quality.evaluate`) — no separate `agent.validation` is ever specified. `Optimization` is **descoped demand-driven** with named re-open conditions: real usage asking for graph restructuring/performance advice AND per-node duration/size evidence in the runtime journal; until then the standing guardrail continues (not-installed, never assumed). | `85-evaluator-validation-optimization-decision-memo.md` |
| DEC-055 | **OQ-007 resolved: the generated-content evaluator is AUTHORED, not migrated** — `agent.generated-content-evaluator` ships as a net-new built-in (the dev/48/50/52/67-4/84 authorship posture) to an approved contract: read-only evidence tools (`node.read`/`node.runtime.read`/`dataflow.read`), findings `{blocker|warn|note, claim, quoted evidence, suggestion?}` with a verdict (`fits`/`fits-with-warnings`/`does-not-fit`) **strictly derived from findings**, ungroundable claims reported as uncertainty. **Authority: none** — `report-only`; its verdict is advisory to users and delegating parents; DEC-054's simulation auto-approve stays exclusively empirical and DEC-028's self-certification firewall stands (RISK-EVAL-001 controls unchanged). The `evaluation-disabled` profile row is retired. | `85-evaluator-validation-optimization-decision-memo.md`, `86-generated-content-evaluator-memo.md` |
| DEC-054 | **Simulation Mode is the default build model** (dev/67-9, assembling 67-5..67-8): the Dataflow Builder's plan drives ONE transition function — per node in topological order, `planned → create → validate (execute-through, self-correcting) → auto-approve on PASS (recorded `approvedBy: simulation-auto`, never silent) → approved`, then the connection stage — exposed as `step` (exactly one action per call) and `auto` (the re-targeted Apply Plan, "Build & validate plan"); the two CANNOT diverge because they share the function. Every transition persists to `builderSession` BEFORE it is emitted (reload resumes exactly); any failure PAUSES with a named `pauseReason` and the pending review — nothing downstream of a failure is generated or created; resume is the same endpoint; cancellation rides the dev/63 dual signals. Structural enabler: a pending PLAN proposal **parks** (`record.planProposal`) while its own per-node content reviews cycle through the single active slot — addressable throughout, completed or replaced, never superseded by its own sequence. The classic bulk apply survives only as the explicit secondary "Apply all without validation". | `67-9-simulation-mode-orchestration-memo.md` |
| DEC-052 | Per-node runtime state is a **server-owned observational journal** (dev/67-2): every `/process*Code` execution best-effort-persists the node's latest outcome — status by the canonical `output.path == ""` predicate (never stderr-nonempty; warnings land there), traceback/stdout tails, output metadata, normalized content digest — under `<project dir>/runtime/` (`DEC-040` FS posture); writes never delay or fail an execution, reads fail open ("never-executed"). Agents reach it via `node.runtime.read` (with a best-effort `contentChangedSinceRun` signal) and the **structure-first `dataflow.read` v2 projection**: ALL edges always survive (node content elided to lengths — the pre-67-2 dump truncated the edge list away on any non-trivial dataflow), runtime statuses ride along, and `params.include: ["content"]` restores the full dump. The composites' declared reads (`graphContext`/`mission`/`installedTemplates`/`targetContext`/`nodeIntent`) gain frontend producers — before 67-2 they composed `null` and ran context-blind. The saved spec stays the single structural truth; the journal is evidence, never authority. **Superseded in part by `DEC-062` (dev/93):** the `installedTemplates` frontend producer introduced here is retired — every template roster an agent sees is composed per run by the packages domain. | `67-2-dataflow-awareness-runtime-journal-memo.md` |
| DEC-050 | Solve streams and cancels — the `DEC-021` **user slice**: the batch is ONE generator (`solve_attachment_stream`; the blocking endpoint drains it — one implementation) yielding `solve_started` → `node_started`/`node_result` per target → `done` over the dev/22 SSE envelope; solved content reaches the live canvas per node (the dev/51 bridge) and the strip's pills advance live (transient overlay, cleared on the terminal event); the terminal state comes from ONE re-guarded finally-write that also runs on client disconnect (`GeneratorExit`) — streamed events are transport, never truth. Cancellation is one predicate with two signals: an in-process stop event (registry keyed by the persisted `solveExecutionId`) plus the durable `cancelRequested` session flag (`POST …/solve/cancel`), checked at node-dispatch boundaries — in-flight children finish and persist (`DEC-021`'s no-replay posture), undispatched targets revert to `pending` (no new status vocabulary; `interrupted` stays with lease expiry). `DEC-021` proper (leases/heartbeats, background execution, the LangChain re-open condition) remains open. *(dev/115, `DEC-073`, 2026-09-08: the single-process slice is DELIVERED — detached re-attachable Solve jobs, `interrupted` reconciliation on read, linked Retry; multi-instance ownership stays `OQ-009`; the LangChain re-open condition was met and declined with recorded criteria.)* | `63-streamed-solve-progress-memo.md` |
...
- LangChain imports must remain inside infrastructure/runtime adapter modules. *(None exist today — `DEC-048`; the rule binds any future re-adoption.)*
- Attachments must be dataflow-owned, not inferred from transient canvas selection.
- Package dependency selection and LangChain version must be pinned and verified before Phase 2 begins. *(Historical gate — `DEC-048` retired the LangChain adapter; no LangChain pin is required unless `DEC-021` reopens the seam.)*
...
| --- | --- | --- | --- | --- |
| OQ-007 | ~~What is the approved source text and output contract for `evaluate_generated_content_prompt`?~~ **RESOLVED by `DEC-055`** (dev/85, owner-approved 2026-08-18): authored net-new to the approved contract; implemented in dev/86. | Prompt/product owner | — | Closed. The original guardrail (never infer/substitute a migration source) was honored: the prompt is a deliberate authorship under the decision, not a fabricated migration. |
| OQ-008 | ~~What are the approved retention, deletion, backup-expiry, export/reveal, and account-closure rules for transcripts/events, prompt drafts…, evaluation suites…, prompt-audit metadata…, publications, crypto-shredding tombstones, and restored copies?~~ **RESOLVED by `DEC-057`/`DEC-058`** (dev/87, owner-approved 2026-08-19; implemented dev/88; rules in `docs/RETENTION.md`): lifecycle-bound deletion, no invented durations (operator-declared retention, undeclared stated as undeclared), truthful live-store deletion language, unpublish-by-default closure, and the governance-artifact contract as v2 build input. | Product + Security/Privacy | — | Closed. The guardrail became mechanical: production release requires the operator's declared backup posture in `.curio/agents-retention.json`; the platform never claims irreversibility beyond its live store. |
...
| OQ-011 | ~~Should the two still-unspecified product agents — `Validation` and `Optimization` — be specified with their own manifests/capabilities or descoped?~~ **RESOLVED by `DEC-056`** (dev/85, owner-approved 2026-08-18): Validation = covered by the shipped family (no new agent); Optimization = descoped demand-driven with named re-open conditions. (`Package Recommendation`, originally flagged with them, shipped via `DEC-035`/dev/84.) | Product + architecture | — | Closed. The not-installed/never-assumed guardrail continues for anything not shipped. |
...
| RISK-EVAL-001 | Stale, self-referential, unapproved, or unbudgeted evaluation falsely certifies a prompt. | Exact input pins, candidate/evaluator separation, independent authorization/budget, stale detection, no automatic activation/release/publish, and the fail-closed unavailable state (originally the OQ-007 gap; OQ-007 is closed by `DEC-055`). |
...
React drawer/project palette/canvas/chat/settings
        ...
        -> AgentRuntime interface
             -> LangChainRuntimeAdapter   (historical — DEC-048: shipped as the direct provider-port runtime)
...
| ~~`evaluation-disabled`~~ | `agent.generated-content-evaluator` | Retired by `DEC-055` (OQ-007 resolved): the evaluator ships report-only with read-only tools — advisory judgment that can never run mutations or self-certify; the DEC-028 platform-quality firewall stands. |
...
> **Supersession note (`DEC-048`):** the LangChain-specific construction below is the July baseline design. The shipped runtime meets the same `AgentRuntime` contracts, normalized events, authorization steps, and review pauses as direct provider-port code (dev/39/41, `DEC-046`/`DEC-048`); no `LangChainRuntimeAdapter` exists. The LangChain seam reopens only with `DEC-021` background execution.
...
- `LangChainModelBridge` is the only module translating provider adapters/configuration into LangChain model objects. *(Historical — `DEC-048`: shipped as the direct provider-port model resolver.)*
...
- Redact secrets and sensitive dataset values before telemetry; the retention, deletion, backup/restore, reveal/export, protected-content, remediation/tombstone, and account-closure policy for transcripts/events plus prompt drafts, evaluations, and audit evidence is defined by `DEC-057`/`DEC-058` (OQ-008 closed — dev/87/88, `docs/RETENTION.md`); the remaining production-release gate is the operator's backup declaration.
...
- Contract: every provider adapter against a shared suite; the runtime port (LangChain retired — `DEC-048`); manifest fixtures; API schemas; execution event ordering/resume.
...
- Phase 2: provider/runtime contracts (LangChain retired — `DEC-048`), strict effective-policy resolution, atomic cost/quota/resource reservations, secret/egress/SSRF rules, authenticated streams, lease interruption/retry, flags/kill switches/telemetry, cancellation, security, and failure injection pass in the approved OQ-009 topology.
...
- Add provider registry/profiles/opaque secrets, OpenAI-compatible Ollama bridge, project-template capability/prompt/delegate/tool resolution, egress checks, project/instance effective policy, reservations/ledger, execution pins, leases/events/retry (the LangChain boundary is retired — `DEC-048`), flags/telemetry, and one Node Explainer chat run.
...
- `REQ-RUNTIME-002`: Executions persist leases/heartbeats; startup reconciliation marks expired nonterminal work `interrupted`, never replays provider/tool calls automatically, and explicit retry creates a new linked execution. *(Gated: durable leases/heartbeats ride the open `DEC-021`/`OQ-009` background-execution decision; the shipped synchronous runtime uses the 15-minute stale guard — `DEC-050`.)* *(dev/115, `DEC-073`: in the single-process topology the job registry IS the lease — a `solving` session whose execution this process does not hold is `interrupted` on first read and Retry is a linked execution; the 15-minute guard stays the outer bound; multi-instance leases remain `OQ-009`.)*
...
9. `feat(agent-runtime): add LangChain boundary, events, cancellation, prompts, tools, and policy snapshots` with tests. *(LangChain boundary superseded by `DEC-048` — delivered as the direct provider-port runtime.)*
