...
**Status: IMPLEMENTED (2026-09-09) on `imp/agentcatalog` — commit 2 landed as `b534f667`: the five validators deleted from `sandbox/util/parsers.py`, `checkIOType` reduced to a documented no-op that stays seeded (worker `_globals_cache` and the zygote unchanged), the worker's three calls and their synthetic payloads removed, the e2e replay call removed and the `nodeType` comment made truthful, `test_sandbox_namespace.py` pins the no-op on top of the #158 pin, `sandbox/tests/test_worker_io_check_retired.py` pins that a Data Loading node returning a dict and a Data Export node returning a value execute under every spelling of their type, `docs/ARCHITECTURE.md` and the legacy `python_wrapper.txt` comment corrected. Sandbox tests 334 passed / 44 skipped (the macOS isolation-fallback test aside); backend four suites 2084 passed / 5 skipped (the environmental stub test aside); `tests/test_frontend` collects 428. Ledgers: DEC-062 and DEC-076 notes, dev/00 row, `BL-P5-20260909-54`, `3.1`, dev/119 F1/F2 closed. Owner smoke of `test_alive.py` with the fixture-booted stack pending. History — Owner decision on the §3.2 question: keep `checkIOType` seeded in the node namespace as a documented no-op (the #158 pin stands unchanged); the validators behind it still go. Commit 1 landed as `0b3ebc11`: the shim restored (12 re-exported names, defines nothing — AST-pinned), `CONSTANTS`/`FLOW_SWITCH` out of `PY_CODE_TYPES`/`CODE_EDITOR_TYPES`, `tests/test_execution/test_workflow_spec_single_source.py` (identity of ten names, no-definition, version strip through the shim, every `CODE_TYPES` member reachable from `NAMESPACED_TO_LEGACY`). Backend `test_execution`+`test_agents`+`test_backend`+`test_packages` 2084 passed / 5 skipped (the environmental stub test aside); `tests/test_frontend` collects 428 tests without a stack.**
