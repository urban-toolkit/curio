...
**Browser (opt-in).** Evaluation mode opens, shows the configured provider and model, lists fixtures, starts a run against the scripted provider, shows a phase, then shows the score with a link to the generated project.
