...
## BL-P4-20260826-17: Agent chat panel slide transition (memo `dev/108`)

- Date / author: 2026-08-26 / Karla
- Status: implemented (owner live re-test pending — re-record and re-run the frame diff)
- Requirements: UI-parity polish — the chat panel is the fourth right-hand drawer and must enter/exit like the Nodes/Datasets/Agents Catalog drawers (`dev/43`); `DEC-042` header, `dev/19/20/25/75/77/80` chat behaviors unchanged
- Design decisions/artifacts: memo `dev/108` — **no new DEC**. Owner report "very flickering" + `Screen Recording 2026-08-26 at 7.06.33 PM.mov`; frame analysis (42.3 fps, 506 frames) showed six single-frame jumps and zero intermediate frames — the panel popped. The Agents Catalog drawer was probed live and slides correctly; the report was the chat panel.
- Tasks: `TASK-P4-chat-slide`
- Risks/questions: none new. Non-modal by design (no scrim/scroll lock) — adding either is a product decision, not parity.
- Design-to-code decision or deviation: **(1)** `AgentDockOverlay` rendered `selected ? createPortal(<AgentChatPanel/>) : null` — instant mount/unmount — and `.panel` had no transform/transition (`dev/46` copied the catalog drawer's sizing, not its motion). **(2)** The dev/43 machine is extracted ONCE as `hook/useSlideDrawerPresentation.ts` (mounted/presented, double-rAF enter, exit timer `motionMs+80`, `finishExit` settle guard that also ignores a stray settle after reopen, reduced-motion snap, jsdom-safe) — deliberately without focus-restore/scroll-lock (modal concerns). The three catalog providers keep their inline copies (follow-up migration). **(3)** The overlay retains the last selected attachment with pinned `index/total` through the exit slide (detach mid-exit cannot repaint "0 / N") and clears it on unmount; switching agents is a content swap in the same DOM node, never a re-slide (no `key`). **(4)** `AgentChatPanel` takes a required `presented`, toggles `.panelPresented`, reports `onExitComplete` from its own `transform` transitionend only while closing, and ignores Escape during the exit slide.
- Files/modules changed: `hook/useSlideDrawerPresentation.ts` (new), `components/agents/attach/{AgentDockOverlay.tsx,AgentChatPanel.tsx,AgentChatPanel.module.css}`, `docs/AGENTS.md`
- Tests added/updated: `tests/hook/useSlideDrawerPresentation.test.ts` (8), `tests/attach/AgentChatPanel.test.tsx` (+3: class toggle, transitionend filter, Escape no-op while closing), `tests/attach/AgentDockOverlay.test.tsx` (new, 7: real panel over a mocked context — mount closed→presented, transitionend settle, timer fallback, content swap on switch, detach mid-exit with pinned header, reopen mid-exit)
- Verification evidence: full `npx jest` → 90 suites / 1020 passed; `tsc --noEmit` clean in touched files (pre-existing errors in untouched `useAgentAttachments.test.ts`, `AgentAttachmentsProvider.test.tsx`, `useAgentCanvasMutations.test.tsx` only)
- Commit/PR: `COMMIT-d7d237a5` (hook + tests), `COMMIT-ebca211f` (panel presentation + CSS), `COMMIT-d2a18527` (overlay adoption + test); docs commit follows
- Follow-up work: migrate `NodeCatalogDrawerProvider`, `AgentsCatalogDrawerProvider`, `DatasetCatalogDrawerProvider` onto `useSlideDrawerPresentation` (deletion-shaped; gate = their existing provider tests).
