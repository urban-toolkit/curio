...
- Follow-up work: ~~`nodeContext.current_input/current_output` currently send empty~~ — **closed 2026-08-27 by `BL-P2-20260827-11` (memo `dev/111`)**. Premise corrected there: legacy passed the node's *declared* I/O types (`data.in/out`), not execution values, and the exec store (`FlowProvider.outputs`) was already reachable from the overlay — the composer simply never read it. P5 composites inherit the pipeline via their declared reads.
- Remaining risks/questions: none new.

## BL-P2-20260827-11: `nodeContext.current_input/current_output` grounded in the live runtime state (memo dev/111)

- Date / author: 2026-08-27 / Karla
- Status: implemented (closes the entry-10 follow-up; loose end tracked by dev/104 §3.1 and `BL-P5-20260804-02`)
- Requirements: dev/38 `inputs.reads` contract (the `nodeContext` read declared by Node Explainer, Node Researcher, Dataset Finder, Generated Content Evaluator), dev/44 grounded-context invariants (live, ephemeral, bounded), dev/67-6 honesty vocabulary (`never-executed`)
- Design decisions/artifacts: memo `dev/111`. **No new DEC** — completes an existing contract. Amends the entry-10 follow-up premise (see above).
- Tasks: `TASK-P2-grounded-inputs` (closure)
- Risks/questions: none new — the summaries are identifiers, types, status, and a truncated error message; a successful output's content is never forwarded.
- Design-to-code decision or deviation: **(1) Premise corrected before design** — legacy `NodeExplanation` sent `data.in/out` (declared `SupportedType` labels), so byte-parity would have shipped two type labels; the exec store was already in the overlay's `useFlowContext()`. **(2) One pure summarizer** (`nodeRuntimeSummary.ts`, exported caps: 240 error chars, 8 merge slots, 64-char artifact ids): `current_input` = upstream `dataType` + artifact id from `data.input` (per-slot for Merge Flow, holes as `empty`) or `no upstream input`; `current_output` = `success: <type> artifact <id>` (artifact from the `outputs` store, type/`outputType` fallback) / `error: <truncated message>[; last artifact: <id>]` / `running` / `success (restored): …` (store-only after reload) / `never-executed`; each with ` (declared: TYPE)` when a type is set. Raw sandbox `dataType` is reported beside the declared label rather than re-mapped — no second copy of the `styles.tsx` type table. **(3) Composer** reads the LIVE ReactFlow node (the Trill does not carry `input/output`) and an optional `AgentCanvasState.outputs`; the other `nodeContext` keys and every other fragment are byte-unchanged (dev/44 parity tests untouched). **(4) Overlay** passes `outputs` from the flow context (one line). Server-side `node_context.py` (delegation path) untouched.
- Files/modules changed: frontend `components/agents/attach/{nodeRuntimeSummary.ts (new),agentRunContext.ts,AgentDockOverlay.tsx}`; `docs/AGENTS.md`; this file; `BL-P5-first-release-orchestration.md` (entry 02 cross-ref); `3.1-Agents-Catalog-Build-Log.md` (remainders list)
- Tests added/updated: `nodeRuntimeSummary.test.ts` (15: none/typed/legacy-path/merge holes/merge cap/malformed/id truncation; never-executed/running/success-store/success-widget/error truncation/error-after-success/structured error/restored); `agentRunContext.test.ts` (key-presence assertions replaced by exact values + 3 grounding cases: store artifact + content never forwarded, no-store fallback, errored run); `AgentDockOverlay.test.tsx` (+1: send composes from live nodes AND the outputs store)
- Verification evidence: frontend `npx jest` full → 1064 passed (93 suites); `tsc --noEmit` clean on touched files
- Commit/PR: `4b3d8cf6` (commit 1, summarizer + tests), `6e454775` (commit 2, composer/overlay wiring + tests), docs in the closing commit
- Issues/regressions discovered: the recorded follow-up premise was wrong on both counts (legacy values; exec-store availability) — an untested claim survived four backlog restatements because the only test asserted key presence.
- Follow-up work: column/schema metadata for `current_output` when the runtime journal captures it (dev/67-6 recorded follow-up — a documented seam, not a promise); the widget's local-only type refinement (`styles.tsx:206-229`) never writes back to `data.in/out` — separate UI concern if ever wanted.
- Remaining risks/questions: `data.output` reaches `getNodes()` via the `useNodeState.ts:28` object mutation; if that is ever refactored away, the store path still yields `success (restored)` and status degrades to `never-executed` — the summarizer is defensive, but the status source should move to a store at that point.
