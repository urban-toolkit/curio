...
- **F6 — the egress budget starves the last candidate row.** **CLOSED 2026-09-09 (owner chose the run-budget option): the run budget is `_RUN_EGRESS_CALLS = _CANDIDATES_MAX_ROWS_PER_LANE × 2` (16) — every external row the card may carry plus one redirect each; the verified Solve loop owns its own `_LOOP_EGRESS_CALLS` (dev/116 `ab87e8fb`); `egress.MAX_CALLS_PER_RUN` (4) now bounds only the model's own web.fetch/web.search calls. Pinned by `TestRunEgressBudget`.** `MAX_CALLS_PER_RUN = 4` is charged per hop (correctly), so one redirect among four external rows leaves the fourth row `refused — budget spent`. Honest, but the user reads a budget refusal on a row that was never checked. Options: a per-row sub-budget, or a run budget sized to the candidate cap plus redirects (e.g. 8). Decide with the owner.
