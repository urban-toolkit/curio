...
## Addendum (2026-08-27) — remainder statuses

Of the eight items in §1.4: `current_input/current_output` wiring **closed** by dev/111 (`BL-P2-20260827-11`); the drawer `Unpublish` control **closed** by dev/107 (`BL-P3-20260826-13`); the `connection` attach target — deferred, then **closed the same day by dev/113** (`BL-P5-20260827-47`, DEC-071: dev/15's existing-edge target for Node Builder with the reviewed `node.insert`; the legacy handle-side suggestions stay a separate, untouched concept). The BL-P5 entry-30 follow-up line and the `3.1` remainders block carry the same status.
